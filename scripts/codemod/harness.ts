...
export class Mod {
  ...
  renameProperty(rel: string, interfaceName: string, from: string, to: string): void {
    ...
    for (const sf of this.project.getSourceFiles()) {
      if (!sf.isSaved() && !this.touched.has(relOf(sf))) this.touched.set(relOf(sf), new Src(this, sf, relOf(sf)));
    }
  ...
  commit(): void {
    ...
    if (FLAG.diff) for (const f of files) console.log(`\n${diff(f.before, f.after, f.rel)}`);
    ...
    if (!FLAG.apply) {
      console.log(
        `\n[${this.name}] DRY RUN — ${files.length} file(s) would change, ${this.journal.length} op(s).` +
    ...
    const restore = (why: string): never => {
      ...
      fail(`${why}\n    ROLLED BACK ${files.length} file(s); originals also in ${backup}`);
    };
    ...
    console.log(`\n[${this.name}] APPLIED — ${files.length} file(s), ${this.journal.length} op(s), verified on disk:`);
