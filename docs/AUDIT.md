...
| C-48 continents | 1 | no tile carries a continent on either engine; Spain's Treasure Fleet (12 route rows), Roosevelt's home-continent strength, Phoenicia's colonies and every Intercontinental clause wait on it |
...
| C-55 a Builder's charge into a wonder | 1 | EFFECT_ADJUST_PLAYER_UNIT_WONDER_PERCENT — a Builder spends a charge to complete 15% of an Ancient or Classical wonder's cost. Neither engine has the verb: a Builder's charges buy improvements and nothing else, and the production queue takes no lump from a unit. The First Emperor's clause waits on it |
| C-52 the harvest | 1 | a Builder never removes a RESOURCE for a lump yield: `builderHarvest` stands on TS with no caller (no driver order, no GPU verb, no wire column), so the Maori's harvest ban refuses something no game can do |
| C-53 an improvement's culture bomb | 1 | EFFECT_ADD_CULTURE_BOMB_TRIGGER — completing a named IMPROVEMENT claims the neighbouring tiles. Both engines bomb from a DISTRICT (`congressCultureBombSeat`, the Preserve, the Border Control Treaty) and neither watches an improvement finish: the Maori's Fishing Boats and the Netherlands' Harbours have nothing to fire on |
| C-50 appeal is map-global | 1 | `tileAppeal` and its GPU plane take no seat, so a clause that changes what a FEATURE is worth to ONE civilization has nowhere to land: Brazil's Amazon ("+1 Appeal to adjacent tiles, instead of the usual -1" from Rainforest) and Roosevelt's National Park appeal. The carrier is a per-seat appeal read — the four consumers (housing, amenities, the Seaside Resort's gold, the National Park's site) each take the asking seat |
...
- **C-48. CONTINENTS.** Weight 1. Neither engine's tile carries a
  continent; the seeder's `layout: 'continents'` shapes land masses without
  naming them. CIV6 keys a dozen roster clauses on it: Spain's Treasure
  Fleet (routes "between continents", EFFECT_ADJUST_TRADE_ROUTE_YIELD_FOR_*
  with Intercontinental), Roosevelt's home-continent Combat Strength
  (REQUIREMENT_UNIT_ON_HOME_CONTINENT), Phoenicia's colonies and the
  foreign-continent settling clauses. A flood-fill of land at seed time
  (`world/mapgen.ts`) with a per-tile id on the wire is the whole carrier.
...
- **C-55. A BUILDER'S CHARGE INTO A WONDER.** Weight 1. CIV6 (The First
  Emperor): "When building Ancient and Classical wonders you may spend Builder
  charges to complete 15% of the original wonder cost." A Builder's charge
  buys an improvement on both engines and nothing else; the production queue
  takes progress from Production and from a Great Engineer, never from a unit
  standing on the site. The carrier is a Builder verb whose target is a
  queued wonder of a named era tier, plus the lump on the queue — the
  engineer's own path is the shape to follow. Qin's modifier is marked open
  against this item.
- **C-52. THE HARVEST.** Weight 1. CIV6 lets a Builder spend a charge to
  REMOVE a resource for a one-off lump of Food, Production or Gold.
  `builderHarvest` is written on TS and `harvestGrant` prices it, but
  nothing calls the verb: the driver never orders it, the wire carries no
  column for it, and the GPU has no arm at all (its `CHOP` removes a
  FEATURE, which is the other half of the pair). So the Maori's
  EFFECT_ADJUST_PLAYER_PREVENT_HARVEST_RESOURCE refusal ships on TS and
  refuses something no game reaches. The carrier is a harvest verb on both
  engines beside the chop: an order column, a legality body sharing the
  chop's early returns, and the lump on the applier. That ban is marked
  open against this item in docs/roster_ledger.json.
- **C-53. AN IMPROVEMENT'S CULTURE BOMB.** Weight 1. EFFECT_ADD_CULTURE_BOMB_TRIGGER
  names an IMPROVEMENT whose completion claims every unclaimed neighbouring
  tile for the builder. Both engines already bomb — `congressCultureBombSeat`
  and CULTURE_BOMB_RANGE carry the Preserve's and the Border Control
  Treaty's — but only off a DISTRICT completing; no path watches a Builder
  finish an improvement. Two roster clauses wait on it: the Maori's Fishing
  Boats (Mana) and the Netherlands' Harbours. The carrier is the same bomb
  body, called from the improvement-complete path on both engines with the
  triggering improvement as a data row.
- **C-50. APPEAL IS MAP-GLOBAL.** Weight 1. `tileAppeal(map, tile, camps,
  ...
  -1", EFFECT_ADJUST_FEATURE_APPEAL_MODIFIER +2) and Roosevelt's "+1 Appeal
  to all tiles in a city with a National Park". The carrier is a per-seat
