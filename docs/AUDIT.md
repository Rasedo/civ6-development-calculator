...
- **C-2. DIPLOMATIC AGREEMENTS.** Weight 1.
  - BUILD: take the RESEARCH AGREEMENT OUT. `DLC/Expansion1/Data/Expansion1_Alliances.xml` carries `<Delete Type="DIPLOACTION_RESEARCH_AGREEMENT" />` and Expansion2 re-ships that file, so by the modinfo load order the action exists in no Gathering Storm game; the live database has no row for it and `DIPLOMACY_RESEARCH_AGREEMENT_BEAKER_PERCENTAGE` 10 is a dead parameter beside it. Both engines built it (`DEAL_RESEARCH_AGREEMENT`, `researchPactsTick` / `_research_pacts_tick`) — pending the owner's word to keep it stylized. What replaced it is the ALLIANCE (`DIPLOMACY_ALLIANCE_TIME_LIMIT` 30, three levels at 320 / 960 XP).
  - BUILD: the PROMISES' engine half — the four rows of `Expansion2_DiplomaticActions.xml` (`DIPLOACTION_KEEP_PROMISE_{DONT_SPY, DONT_CONVERT, DONT_DIG_ARTIFACTS, DONT_SETTLE_TOO_NEAR}`, FavorCost 30, GrievancesForRefusal 25, GrievancesPerIncursion 25) and `DIPLOACTION_DECLARE_WAR_OF_RETRIBUTION`'s `RequiresBrokenPromise="true"` are a grievance ledger and a casus belli, both sourced. Whether our own scripted driver ever asks for a promise or breaks one is the DRIVER's design, not the install's; the mission's "small positive bonus in your relationship" is an opinion delta and stays out (owner 2026-09-20: the engine, not the game's AI).
...
- **C-26. CIVILIZATION ABILITIES — THE RESIDUE.** Weight 1.
  The census is `docs/roster_ledger.json`, read as `docs/ROSTER.md` says (`shipped` on 338 of 343 modifiers, three `open:` rows under C-64, two `AI:` rows — the game's diplomatic-action preferences, outside an engine model). Unique units are C-78, unique infrastructure C-79.
...
- **C-64. A SEAT HAS NO MAJORITY RELIGION.** Weight 1.
  - BUILD, measured over eight spread draws, both halves. A CITY's majority is the argmax of FOLLOWERS over all groups with THE UNCONVERTED COUNTED AS ONE OF THEM, ties broken by that group's total PRESSURE — the decider was one more spread that reversed the pressure order and flipped the answer to the religion with the higher id and the later arrival — and the winner must also satisfy `2 * followers >= population` or the city has no majority at all. A SEAT's is the religion STRICTLY MORE THAN HALF of its cities follow; cities with no majority still count in the denominator, so one-of-two is not enough.
  - BUILD, the readers: both engines already compose the seat half (`dominantReligionOf` / `_dominant_religion`, written for tourism and read by the concert) and neither wires it to the three roster rows that want it (`TRAIT_CITY_STATE_TOKEN_SAME_RELIGION`, `TRAIT_COMBAT_BONUS_OTHER_RELIGION`, `TRAIT_GAINS_FOUNDER_BELIEF_MAJORITY_RELIGION`) or to `conquistadorConvert`, which falls back to the captor's own founded religion and says so. The CITY half is `city.followedReligion`, a pressure argmax with no unconverted group and no half-population gate.
