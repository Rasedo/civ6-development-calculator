...
- **C-81. THE TILE SWAP'S REACH.** Weight 1.
  ...
  - FORUM (lowest tier — a lab line, not a build): players report the swapped tile must be ADJACENT to a tile the claiming city already owns (and within its radius 3), never next to a city centre, never under a district or wonder (even one started), and never while a conquered city is not yet ceded (forums.civfanatics.com threads 644094 and 642399; a Steam guide). The adjacency clause is in neither engine (`swapTileOk` / `_swap_tile_ok` test the radius only); the lab 4 scene met every listed clause for plot (18,14) and was still refused, so the reported list is not the whole rule.
