...
silently mirroring: a missing `rngrc` site, the seatPhase movement
...
- #71 ROUND RESIDUALS deltas (2026-07-26). Batch 1: A-5r 2 (97% —
  ...
  **B-27 TAIL ANALYSED 2026-07-29 (#79) — NOT SHIPPED, and here is exactly why.**
  ...
  So the B-27 comment at phase.ts ("pass the ACTING unit's type so a
...
- #71 FLAG SWEEP (2026-07-27): five of the six inert `_LIVE` flags are now
  ...
  **FLAGS RETIRED (2026-08-09, #104): RIVAL_TILE_BUY_LIVE and
  APOSTLE_BUY_LIVE are DELETED, not flipped — the #103 ruling made every
  gold/faith purchase and the levy a wire DECISION, so the scripted arms
  (and their inert halves) are gone. The record now carries `buy` kind 3
  [3, tile, centre], `buyFaith` [[4|5|6, centre]...] and `levy` csIndex;
  both engines re-validate at their own phase positions, candidates come
  from shared legality bodies (_seat_tile_buy_candidate /
  _seat_faith_buy_candidates / _seat_levy_candidate and their TS driver
  twins), and the serve handshake's buys tripwire compares all seven
  candidate families. The apostle lifecycle drift named above is now
  reachable the moment the driver emits kind 6 — it will resurface in the
  first post-freeze serve hunt, at its causal turn, with attribution.**
...
- **B-19/#39 PROJECT RATES — FIXED 2026-07-29 (#79).** The last Festival
  ...
  **NARROWED TO RELIGION FOUNDING (2026-07-28) — a COUNT vs a SET.**
   ...
   * TS (core/phase.ts ~975-990) builds real lists and requires
  ...
  **ROOT-CAUSED 2026-07-29 (#79): rGScore1 IS A LOST RELIC ON A CITY TRANSFER.**
  ...
  constructors (`phase.ts` defected / flipped, `combat.ts` captured) and never
...
- **#49 NEUTRAL-RIVAL TARGETING — FIXED 2026-07-29 (#79).** The #78 attack-target
  ...
  (phase.ts) and CS CAPTURE (combat.ts), by which point the tile belongs to a
  ...
  **THE TS TWIN IS `hostileUnitAct` (core/combat.ts), NOT `walkPath`** — walkPath
  is never called from phase.ts at all; the GPU comment cites it only for the
  ...
  **ROOT CAUSE CONFIRMED (2026-07-28): the own-city target SUPPRESSES the march.**
   ...
   * "TS's tile lacks the CITY_CENTER district its predicate keys on" — NO.
     Initial rivals are built through foundRivalCity (phase.ts:258-287), which
     sets `tile.district = 'CITY_CENTER'` (phase.ts:202).
  ...
  **CONSOLIDATED HUNT STATE for BOTH score latents (2026-07-28, #78).** Two
  ...
  Both scores are `pop * N + sum(weighted city YIELDS)` (empireScore, now the
  one implementation in cpu/core/score.ts). A gap of exactly 1.0 therefore
  ...
  **RIVAL SIDE NOW IMPLEMENTED TOO (2026-07-28) — all four sites done.**
  phase.ts mirrors city.ts's waterMillBonus post-selection over the worked
  ...
  **DESIGN SETTLED (2026-07-28), so the next pass executes rather than
  ...
  `completedWonders(state, city)`, phase.ts:910 over `rc.wonders` filtered by
  ...
  **IMPLEMENTED (2026-07-28), all four sites.** greatPeople.ts gains
  ...
  building could not express. game.ts and phase.ts supply the resolver because
...
- A-5 (remainder). Scripted rivals spend gold on ONE building per civ
  per turn (the A-5 block in `seatPhase` (phase.ts); `_rival_phase`
...
- A-11 (90% — 2026-07-18, task #41 stage 2). Rivals RUN domestic trade
  ...
  by the deterministic best-dest scan (`seatPhase` creation block /
...
- A-12. RESOLVED (2026-07-19, task #64 ROUND B8 slice L — rival levy + rival
  ...
  accrual with the adopted-government tier (`seatPhase` CS block /
  ...
  * LEVY — the `levyUnits` twin for rivals, in the `seatPhase`
  ...
  * QUESTS (zero-draw) — one deterministic quest per (rival, CS):
    ...
    `issueRivalQuest`/`rivalQuestSatisfied` (phase.ts) /
...
- A-19. **RESOLVED (2026-07-20, task #55 S1/S2 — brief
  ...
  (`RivalCiv.atWarRivals` + `civsAtWar`/`setRivalWar` in phase.ts;
...
- A-22. **RESOLVED (2026-07-27).** Rivals now assign SPECIALISTS, and the GPU
  ...
  `computeCityStats`, but `rivalCityYields` (phase.ts) never reads
...
- A-23. **RESOLVED (2026-07-27).** The rival worked-tile scan is now
  ...
  CIV-level: `rivalCityYields` (phase.ts) ranks
...
- A-24. RESOLVED (2026-07-19, ROUND B10 slice R, task #66). Rival
  ...
  `assertRivalRegistryCoherent` (phase.ts), env-gated
...
- B-8. RESOLVED-minus-MP (2026-07-19, ROUND B7 slice G, task #63):
  ...
  loop). `applyGreatPersonEffect` + the phase.ts claim loop (GPU
...
- B-26. Map/barbarian fidelity: no cliffs (no such concept in
  ...
  **ERA LADDER RESOLVED (2026-07-19, ROUND B10 slice B, task #66)**: all
  ...
  (`state.turn > 60 ? SPEARMAN : WARRIOR` in phase.ts) is untouched.
...
- B-21. RESOLVED-minus-descoped-rows (2026-07-20, ROUND B8 slice K).
  ...
  and the `rc.buildings` loop (rival, phase.ts) — inheriting
...
- B-23 (~85% — #71 batch 2 2026-07-26 landed ROADS, one of the two
  ...
  push (`phase.ts`), and the filter runs UNCONDITIONALLY — player via
  `expirePlayerRoutes` (game.ts endTurn, after seatPhase), rivals via the
...
- B-24 (70% — 2026-07-20, task #68, brief gpu/docs/design/GOVERNORS_DESIGN.md;
  ...
  **B-24 GOLDEN-AGE DEDICATION BONUSES LANDED 2026-07-29 (#79).** A Golden age
  ...
  METHOD: TS freezes a unit's MP as STATE (spawn -> refreshUnits -> seatPhase
...
- G-5. RESOLVED (2026-07-19, ROUND B10 slice-H, #66). The class: a
  ...
  all bit-identical. ROOT: `transferCityToRival` (phase.ts) built the
...
- G-10. **RESOLVED (2026-07-26, #70)**. `captureCityState` and
  ...
  (game.ts) and `foundRivalCity` (phase.ts), which both do. So an
