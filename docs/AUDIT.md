...
| B-66 formations | 1 | the merged unit's hit points and spent turn are STYLIZED (owner ruling 2026-09-04); a direct-trained formation's strategic-resource charge is modelled at the single unit's; an escort formation is a PAIR here, and a dragged rider lifts no fog |
...
| C-33 the Giant Death Robot's remaining abilities | 1 | every published clause ships; the Jump's cost is STYLIZED (the hex's own — owner ruling 2026-09-04) and its five-hex Range is a verb the action space lacks |
...
4. **C-33 — the Jump action's cost.** RULED 2026-09-04, STYLIZED: the
   Jump enters the mountain at the hex's own movement cost. What stays
   open is the five-hex RANGE, a verb the action space lacks — see the row.
...
11. **C-16 — the released spy's level.** RULED 2026-09-04, STYLIZED: the
    spy that comes home is the one that was caught, at its level — the
    cell holds levels, not a count. Shipped; see the row.
...
- **B-66. FORMATIONS.** Weight 1 — the mechanic SHIPS; two tails stay open.
  ...
  - **WHAT THE MERGED UNIT KEEPS BEYOND THE VETERAN'S RECORD IS THIS
    ...
    the promotions and experience of — and end its turn. STYLIZED by owner
    ruling 2026-09-04: the shipped reading stands. Beside it the SOURCED
    half, re-read the same day: a Corps waits on Nationalism and an Army on
    Mobilization (`FORMATION_CIVIC`), and a direct-trained formation costs
    UNIT_CORPS_COST_MODIFIER 1.5 / UNIT_ARMY_COST_MODIFIER 2.0 of the unit
    (GlobalParameters.xml) — which CORRECTED `FORMATION_COST_MULT`'s army
    step from 2.25, a figure taken off the Military Academy's civilopedia
    text; the table outranks the prose.
...
- **C-16. THE SPY'S SECOND HALF.** Weight 2. The Spy, its capacity, the
  ...
  - **A RELEASED SPY IS THE SPY THAT WAS CAUGHT.** SHIPS 2026-09-05,
    STYLIZED by owner ruling 2026-09-04. The cell ships: a caught spy is
    "imprisoned, but not killed", held by the seat whose city made the
    catch, still counted against its owner's capacity ("if you've trained
    the maximum number of Spies possible, you cannot train a new Spy to
    replace one that gets captured"), and traded back through C-2's table
    to arrive "immediately returned to the original owner's Capital". No
    source publishes the level it returns at (re-searched 2026-08-31); the
    best reading is that the SAME spy comes home, so the cell now holds
    LEVELS rather than a count (`spyHeld` as a level list per owner ->
    captor; `seat_spy_held [B, pw, pw, level]` counts by level) and the
    released spy is spawned at the level it was caught at. When one captor
    holds several, the HIGHEST level goes home first — the real deal names
    a spy, this model ranks them (`releaseSpy` / `_spy_cell_release`). The
    promotions themselves are still not carried: a level-3 spy comes back
    level 3 with no picks made, the same shape a freshly levelled spy has
    before its offers. `spyHeldLevels` compares the lists. Bar:
    `spy_release_level` on both engines.
...
- **C-73. A LEGACY CARD PAYS THE WHOLE GOVERNMENT. CLOSED 2026-09-05 — nine of
  nine channels ship, and the driver's legacy-first style reaches them.** Found while
  ...
  THE LAST TWO CHANNELS SHIPPED 2026-09-05: `goldPrice` / `faithPrice` and
  `_gold_price` / `_faith_price` — one composer per purse per engine —
  apply the accrued percent where every purchase is priced and paid: a
  building, a unit, a settler, a worship building, a religious unit, the
  Monumentality civilians, a Naturalist, a Rock Band, a class building, and
  the driver's affordability twins on both sides. READING: an upgrade, a
  tile and a patronage are not "purchases" in the card's sense and pay
  full price. The nine `TRAIT_*_BONUS_RATE` rows ship with it. Bar:
  `legacy-discount` (TS) and `legacy_accrual`'s discount case (GPU) — a
  stored Merchant Republic legacy at 30 turns prices a 100-gold purchase at
  98, Theocracy's the faith one.
  WAS OPEN, two channels: goldPurchases and faithPurchases are DISCOUNTS,
...
- **C-56. A TRADE ROUTE'S RELIGIOUS PRESSURE. CLOSED 2026-09-05.** CIV6
  (GlobalParameters): RELIGION_SPREAD_TRADE_ROUTE_PRESSURE_FOR_DESTINATION
  1.0 and _FOR_ORIGIN 0.5 — a live Trade Route carries its ORIGIN city's
  religion to the destination each turn and the destination's back at half
  strength; CIV6 (Dharma, EFFECT_ADJUST_PLAYER_TRADE_ROUTE_RELIGIOUS_PRESSURE
  Origin:true, Destination:true, Amount:100): +100% on India's own routes,
  both ends. Both engines' per-turn pressure body now carries the term
  (`spreadReligiousPressure`'s route pass / `_route_pressure_terms`), keyed
  by the RECEIVER so the Citadel of God and the Religious alliance masks
  apply to it exactly as to city pressure; a city-state destination takes
  the pressure into its own row and follows nothing (a minor's follow set
  is not modelled on either engine). READING recorded: the accumulator is
  an integer, so the half-point lands on EVEN turns (`routePressureShare`
  / `_route_pressure_share`) — India's doubling makes it a whole point
  every turn. `ROUTE_PRESSURE_ROWS` had been loaded on the GPU with no
  reader (the rows-behind-an-early-return class); this is its reader.
  Bar: `route_pressure` (GPU) and the route case in `religion-trade` (TS)
  — 1 down the route and the half back on an even turn, nothing back on
  an odd one, 2 and 1 for India, nothing back from a destination that
  follows nobody; the pair 9209 / 9092 green to 200.
...
- **C-58. A DEFEATED CAVALRY UNIT MAY BE CAPTURED. CLOSED 2026-09-05, the
  curve STYLIZED by owner ruling 2026-09-04.** CIV6 (Mongol Horde): "All
  cavalry class units gain +3 Combat Strength and a chance to capture
  defeated enemy cavalry class units." SOURCED: the permission —
  `TRAIT_CAVALRY_CAPTURE_CAVALRY_MODIFIER` is EFFECT_ADJUST_UNIT_COMBAT_
  UNIT_CAPTURE `CanCapture: true`, a boolean; the strength; and ONE number
  the install carries beside it, COMBAT_BASE_CAPTURE_STRENGTH_DIFFERENCE
  20 (GlobalParameters.xml). The DLL's curve through that number is not
  readable, so it is this model's, anchored on the sourced base: an even
  fight is a coin flip and the chance moves linearly, certain at +20
  Combat Strength and nothing at -20 (`captureRoll` / `_capture_roll`,
  `pct = round(50 + 5q / 20)` on the same quantized q the damage roll
  uses). STYLIZED beside it: the captured unit arrives at
  `CAPTURED_UNIT_HP` 25, keeps its promotions, experience and formation,
  and a passenger at sea is never captured.

  The hook is the ONE place each engine decides a melee loser's fate: the
  roll is drawn right after the two damage rolls on both engines (before
  the experience award, which may draw a promotion offer), and a capture
  is a DEFEAT for the experience, the weariness and the victor-survives
  rule, and NO death for the kill event, the disciples, the dig, the
  post-kill heal and the advance — the tile is now held by the captor's
  own unit. The row is `CAPTURE_ROWS` (leader GENGHIS_KHAN, the light and
  heavy cavalry classes on BOTH chassis), on the wire under `uniques`.
  Bar: `capture_cavalry` on both engines — the carrier captures at a
  patched certain curve, a non-carrier never, a non-cavalry chassis on
  either side never, the unit arrives at 25 under its captor's flag at the
  end of the roster, and the stream advanced by exactly one draw.
  REACH: no fixture seats Genghis Khan, so the gate never fires it; the
  lanes are the whole bar.
...
- **C-38. A CITY-STATE'S CITY DEVELOPS HALFWAY.** Weight 1. The minor
  ...
  no source publishes a rate (STYLIZED by owner ruling 2026-09-04: the best
  reading is the one shipped, no alternative exists to prefer) — and a
  fixed ladder spends it: Ancient Walls,
...
- **C-46. RELIGIOUS PRESSURE IS ON THE INSTALL'S SCALE. CLOSED 2026-09-05.**
  ...
  MAJORITY rather than "one past the strongest other". The trade route's
  pressure followed the same day (C-56).
