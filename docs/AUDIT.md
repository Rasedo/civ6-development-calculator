...
- **C-64. A SEAT HAS NO MAJORITY RELIGION.** Weight 1.
  - BUILD, the readers: both engines already compose the seat half (`dominantReligionOf` / `_dominant_religion`, written for tourism and read by the concert) and neither wires it to the three roster rows that want it (`TRAIT_CITY_STATE_TOKEN_SAME_RELIGION`, `TRAIT_COMBAT_BONUS_OTHER_RELIGION`, `TRAIT_GAINS_FOUNDER_BELIEF_MAJORITY_RELIGION`) or to `conquistadorConvert`, which falls back to the captor's own founded religion and says so. The CITY half is BUILT (`followedReligionOf` / `_followed_religion`: followers by largest remainder over the religions and the unconverted, the most followers wins, ties by pressure, at least half the citizens); the seat half is `dominantReligionOf` (strictly more than half of the cities, no-majority cities in the denominator) — what is left is the WIRING: Tamar's duplicate token (`MODIFIER_PLAYER_ADJUST_DUPLICATE_INFLUENCE_TOKEN_WHEN_SAME_RELIGION`, Amount 1, on the minor's followed religion), Philip II's +5 (`PHILIP_II_COMBAT_BONUS_OTHER_RELIGION`, `REQUIREMENTS_OPPONENT_IS_OTHER_RELIGION`), Mvemba's founder beliefs of the majority religion, and the conquistador's convert.
