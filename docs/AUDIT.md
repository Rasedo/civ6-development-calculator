...
| B-88 a stripped feature's harvest | 1 | a harvest on a chopped or soil-painted plot restores the Woods-era job values (BUILD) |
...
| C-41 Volcanic Soil | 1 | the severity roll (C-74); the resource loss, the improvement's fate, Marsh / Oasis and built plots on the ring (LAB) |
...
- **B-88. A STRIPPED FEATURE'S HARVEST.** Weight 1.
  - BUILD: a plot whose Woods was chopped or painted over by Volcanic Soil and that still carries a resource gets the Woods-era farm / mine / lumber job values back when that resource is later harvested, on both engines (the chop path and `_paint_soil` both strip the feature but the harvest rebuilds the plot's job values from the starting feature). The harvest must rebuild from the plot as it stands.
