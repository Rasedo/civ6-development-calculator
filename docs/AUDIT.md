...
| C-51 the Terrace Farm | 1 | the Inca's unique improvement is absent from `IMPROVEMENTS`, so its own yields and the five EFFECT_ADJUST_TERRAIN_YIELD_FROM_ADJACENT_IMPROVEMENTS rows that pay a MOUNTAIN +1 Food per adjacent Terrace Farm have nothing to count; the table already carries `uniqueTo`, an `adjacency` list and a fresh-water clause, so the row itself is the whole carrier |
...
- **C-51. THE TERRACE FARM.** Weight 1. The Inca's unique improvement is
  not in `IMPROVEMENTS`, so the five rows that pay a MOUNTAIN "+1 Food for
  every adjacent Terrace Farm" (EFFECT_ADJUST_TERRAIN_YIELD_FROM_ADJACENT_
  IMPROVEMENTS) count nothing, and the improvement's own yields are unpaid.
  The table already carries `uniqueTo`, an `adjacency` list and the
  fresh-water and hills clauses the Terrace Farm needs, so the data row plus
  a terrain-yield-per-adjacent-improvement family is the whole carrier. The
  rows are marked open against this item in docs/roster_ledger.json.
