...
mirror `seatPhase` (cpu/core/phase.ts:1353) has the same structure plus its
...
- Commit `gpu/tools/profile_step.py`: cProfile driver with (a) scripted/f64
  ...
  (guard-storm metric).
- Commit `tools/cpu/perf-turns.ts`: headless TS driver, ~3 fixture seeds ×
...
### Stage T1 — TS modifier cache + trace-path hoists (parallel-agent-safe) — est. 30-50% off TS seatPhase; shrinks gpu-gate TS-replay wall
...
1. **Self-validating value-key cache** in `getRivalModifiers`
   ...
   techs/civics are append-only (pushes at phase.ts:1872/1901; nothing
   removes), and the key covers every input incl. the seat — so writes
   outside phase.ts (combat captures, transfers) are captured with zero
   call-site changes. Kills the worst waste: 2 rebuilds per border-growth
   `while` iteration (phase.ts:1781) + 3-4 per city per turn
   (1034/1131/1708).
...
| 3 | T1: TS mods cache + hoists | low | 30-50% of TS seatPhase |
...
- `src/core/effects.ts` — `getRivalModifiers` 391-410, `withFollowerBelief`
  448-466
- `cpu/core/phase.ts` — `seatPhase` 1353, call sites
