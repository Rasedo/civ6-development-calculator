...
phase.ts, GPU `_city_totals` + BOTH `_rival_city_yields` paths, the
...
the spec itself was wrong: TS `seatPhase` RE-RESETS every rival unit's
...
FIX: `seatPhase` now applies the aura and rewrites `movesFull`, and the
...
- **S2 aura at city/CS sites — TS SIDE COMPLETE (uncommitted), GPU SIDE
  ...
  defender sites (pcstk/pestk in combat.ts, rcstk/restk in phase.ts) via
...
- phase.ts rival walls strike defCS — k=rcstk
- phase.ts rival Encampment strike defCS — k=restk
