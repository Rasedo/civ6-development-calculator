...
- Repair: extend `builderRepair` (units.ts) and the rival repair in
  `_rival_builder_actions`'s TS twin (`seatPhase` builder block) to
...
- `transferCityToRival` (phase.ts) / `_transfer_city_to_rival`:
  rival takes a player city — carry buildings (minus PALACE),
  districts, wonders into the rc planes.
- `transferRivalCityToRival` (phase.ts) + its GPU twin (the loyalty
