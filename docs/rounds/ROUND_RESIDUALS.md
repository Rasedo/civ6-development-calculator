...
* #71 `engine.py` + baseline `src/core/units.ts`, `phase.ts`,
...
barbarianPhase / disasterPhase / seatPhase, difference the rng each
...
`src/data/units.ts` cannot simply be reverted — phase.ts references
UNITS.APOSTLE — so revert PAIRS. Suggested order, cheapest first:
 ...
 c. `cpu/core/phase.ts` (the largest delta).
...
3. Movement: `moveCostInto` becomes `moveCostInto(from, to)` — a
   ...
   sites (units.ts x3, phase.ts x4, combat.ts x1) and the GPU's
