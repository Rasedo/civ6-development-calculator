...
- **A-5r (rival gold-buy units/settlers) — NOT STARTED.** Ran out of budget
  ...
  higher-risk. Next step: extend the A-5 building-buy block in `seatPhase`
  (phase.ts) + `_rival_phase`/`apply_rival_actions` (engine.py) with the
...
`hostileUnitAct` (phase.ts) while the GPU twin iterates unit SLOTS, so a
...
- **B-18 rival enhancer claiming + GPU enhancer race.** The slot + catalog
  ...
  (phase.ts) gated on `religionFounded && !enhancerClaimed && prophets>=2`
...
(city.ts `computeCityStats`) and, symmetrically, per rival (phase.ts
...
its seatPhase / `_rival_phase` block top — both read last turn's war state,
...
(`seatPhase`, cpu/core/phase.ts; `_rival_phase`, gpu/civ6gpu/engine.py)
