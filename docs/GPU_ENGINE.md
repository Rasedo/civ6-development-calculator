...
1. **Scripted parity.** `npm run seed` writes engine-free worlds and `npm run export` compiles them through the **real TS engine**
...
```bash
npm run seed && npm run export  # 1. worlds (engine-free), then compiled planes
