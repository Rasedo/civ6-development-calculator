...
export interface BuildingDef {
  ...
  maintenance?: number;
  /** flat combat XP a unit TRAINED or PURCHASED in this
...
const rawList: BuildingDef[] = [
  ...
  // Ancient Walls — the one defensive tier this stage. A plain
  ...
  // ranged strike) lives in combat.ts/the other seats.ts, not in the yield pipeline.
  ...
  // --- Encampment ------------------------------------------------------------------
  // trainXp: units trained/purchased here start with this XP
  ...
  // The GS Civilopedia's Arena entry reads "+1 Culture" and "+2 Amenities
  // from entertainment".
  ...
  // SOURCING SWEEP: amenities 2 (approximate) -> 1, the GS
  ...
  // exists in either engine — and is a recorded residual, not folded into the
  // base.
...
/**
 * Buildings held out of every SCRIPTED pick (seat queue, seat gold purchase,
 * the exporter's building table) while their support channel is still missing
 * on one engine. The UI path (`availableBuildings`) is deliberately NOT gated.
 *
 * EMPTY today — the regional-effects channel that needed it has shipped. The
 * plumbing stays for the next inert-first stage.
 */
