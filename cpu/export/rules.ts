...
import { ROUTE_CHAIN_MAX, TRADE_ROUTE_RANGE_LAND, TRADE_ROUTE_RANGE_SEA, CITY_STATE_ROUTE_GOLD, CITY_STATE_ROUTE_SPEC, TRADE_ROUTE_DURATION, PLUNDER_ROUTE_GOLD, TRADER_GUARD_RADIUS, TRADE_WALK_EXPIRY_RAIL } from '../core/trade';
...
import { AMENITY_TIERS, CITY_POP_PER_AMENITY, amenityTierIndex, CIVIC_UNLOCK_MAX_COST, CIVIC_UNLOCK_PER_TURN_DROP, CIVIC_UNLOCK_MIN_COST, PALACE_CITY_CS, GARRISON_CITY_CS, ENVOY_CITY_CS } from '../data/constants';
...
export function buildRules() {
  const rules = {
    ...
    seats: {
      ...
      // the pair, per world era (`eraUnitOfClass`), then the recurring
      // grant's period, class weights and chassis per class per era (-1 where
      // the era has none)
      freeCityDefense: FREE_CITY_DEFENSE,
      freeCityPairCount: FREE_CITY_PAIR_COUNT,
      freeCityPair: ERAS.map((_, e) => Object.keys(UNITS).indexOf(eraUnitOfClass('MELEE', e) ?? '')),
    ...
    combat: {
      ...
      // a city centre's standing terms beside its base and walls: the
      // Palace's, a garrison's, and a city-state's per envoy
      palaceCityCs: PALACE_CITY_CS,
      garrisonCityCs: GARRISON_CITY_CS,
      ...
      // PATROL: how far from its hex a deployed fighter intercepts, and what
      // each other patrol in reach adds to the one that fights
