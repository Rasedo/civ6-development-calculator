  ...
  const rules = {
    ...
    projects: {
      rows: Object.values(PROJECTS).map((p, _i, all) => ({
        ...
        // the DECOMMISSION rows: the building this project CONSUMES, and the
        // Climate Accords window that unlocks it (`UnlocksFromEffect`)
        cb: p.consumesBuilding ? (buildingIdx.get(p.consumesBuilding) ?? -1) : -1,
        ao: p.accordsOnly ? 1 : 0,
      })),
    ...
    disasters: {
      ...
      decommissionScore: COMPETITION_DECOMMISSION_SCORE,
