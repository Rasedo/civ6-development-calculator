...
import { PREVAILING_WINDS, STORM_MOVEMENT, FLOOD_WEIGHT, FLOOD_DESTROY_P, FLOOD_DISTRICT_P, FLOOD_BLDG_P, FLOOD_POP_P, FLOOD_DAMAGE_LO, FLOOD_DAMAGE_HI, FLOOD_FERT_FOOD, FLOOD_FERT_PROD, floodTerrainColumn, ERUPTION_WEIGHT, SOIL_PAINT_P, SOIL_REPLACES, KILIMANJARO_FEATURE, KILIMANJARO_WEIGHT, KILIMANJARO_SOIL_P, DROUGHT_WEIGHT, DROUGHT_DURATION, DROUGHT_HEXES, ACCIDENT_WEIGHT, ACCIDENT_MIN_TURN, ACCIDENT_FALLOUT, ACCIDENT_DISTRICT_P, ACCIDENT_POP_P, STORM_EVENTS, STORM_FAMILIES, STORM_UNIT_ROWS } from '../data/disasters';
import {
...
export function buildRules() {
  const rules = {
    ...
    disasters: {
      ...
      // the per-plot Volcanic Soil chance by eruption severity, and the
      // feature ids the soil replaces
      soilPaintP: [...SOIL_PAINT_P],
      soilReplaces: SOIL_REPLACES.map((f) => featIdx.get(f) ?? -1),
      // Kilimanjaro's eruptions: the per-plot paint chance by row, and the
      // feature id whose plots are their sites
      kilimanjaroSoilP: [...KILIMANJARO_SOIL_P],
