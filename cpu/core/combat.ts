...
function xpEligible(unit: Unit): boolean {
  // CIV6 (Giant Death Robot): "Cannot earn experience or Promotions."
  if (UNITS[unit.type]?.gdr) return false;
  ...
  if (defender.embarked) {
    ...
    return embarkedDefenseCS(state, defender.seat) + formationCS(defender) + convoyCS(state, defender) - fuelShortCS(state, defender) + chassisDefendCS(defender, vs)
      ...
      + (vs ? barbarianCombatCS(state, defender.seat, vs.attacker.seat)
        ...
        + allianceWarCS(state, defender.seat, vs.attacker.seat)
        + rosterCS(state, defender, vs.attacker.seat, vs.attacker.hp, false) : 0);
  }
  const tile = state.map.tiles[defTileIndex];
  return (
    ...
    gdrArmorCS(state, defender, vs?.attacker.type) +
    congressUnitCS(state, defender) + governmentUnitCS(state, defender)
  );
}

/** CIV6 (Reinforced Armor Plating): "+10 Combat Strength when defending
 *  against land and naval units" — an air strike is neither, and a city's own
 *  shot is neither, so both go without it. */
export function gdrArmorCS(state: GameState, defender: Unit, foeType?: string): number {
  if (foeType === undefined || UNITS[foeType]?.air) return 0;
  return gdrHas(state, defender, 'REINFORCED_ARMOR') ? GDR_ARMOR_PLATING_CS : 0;
}
 ...
 *  both melee and ranged attacks and when defending.)" — this is the adder,
 *  wherever the robot meets a city; the effectiveness half is the -17 that
 *  `cityRangedStrength` stops taking off. */
export function gdrBeamCS(state: GameState, unit: { type: string; seat: number }): number {
 ...
 * holds.
 */
export function cityRangedStrength(state: GameState, attacker: { type: string; seat: number }, outerHp: number): number {
  const d = UNITS[attacker.type];
  if (d?.bombard !== undefined) return d.bombard;
  // CIV6 (Particle Beam Siege Cannon): "Ranged attacks against Cities and
  // Encampments are 100% effective" — the penalty waived, and the +30 rides
  // in beside it wherever this chassis meets a city.
  const beam = gdrBeamCS(state, attacker);
  return (d?.ranged?.strength ?? 0) + beam
    - (beam > 0 ? 0 : rangedCityPenalty(attacker.type, outerHp));
}
...
/**
 * A CITY CENTRE'S STANDING STRENGTH — one rule for every holder: a major's
 * city, a Free City and a city-state's one city (`minorCity`). The combat
 * preview's DEFENSES lines, read term by term (`PALACE_CITY_CS`):
...
export function healOnEliminate(state: GameState, victor: Unit): void {
  ...
  // CIV6 (Tomyris): the roster's "heal after defeating a unit" rides here
  const n = seatBuildingSum(state, seat, 'healOnKill') + getModifiers(state, seat).postKillHeal;
...
function assaultAtkCS(state: GameState, attacker: Unit, targetIndex: number): number {
  ...
  return (
    ...
    + rosterCS(state, attacker, tileSeat(state.map.tiles[targetIndex]), null, true)
  );
}
...
function encampOuter(state: GameState, tile: Tile): number {
  const held = cityAtTile(state, tile);
  if (held) return encampOuterPool(state, held, tile);
  const cs = isCityStateSeat(tileSeat(tile)) ? state.cityStates?.find((c) => c.seat === tileSeat(tile)) : undefined;
  if (!cs) return 0;
  const max = wallsMax(state, { buildings: cs.buildings ?? [], seat: cs.seat });
...
export function encampmentDefense(
  ...
  // CIV6 (Encampment): the district fights "similar to the parent City
  // Center, excluding any bonus obtained for a Garrisoned unit" — its city's
  // standing strength without the garrison, walls tier included, whoever
  // holds it. A district no city holds stands on the holder's base alone.
  if (isCityStateSeat(tileSeat(tile))) {
    const cs = state.cityStates?.find((c) => c.seat === tileSeat(tile));
    if (!cs) return null;
    return { defCS: centreStrength(state, minorCity(cs), false) };
  }
  const owner = seatOf(state, tileSeat(tile));
  if (!owner) return null;
  const held = cityAtTile(state, tile);
  return { defCS: held ? centreStrength(state, held, false) : holderStrength(state, owner.seat) };
}
