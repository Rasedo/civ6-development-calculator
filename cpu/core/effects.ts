...
export function computeUnlocksIn(
  ...
  buildingPrereq: readonly BuildingPrereqRow[] = [],
): Unlocks {
  const u: Unlocks = {
  ...
  for (const r of buildingPrereq) {
    const has = r.tech !== undefined ? research.techs.includes(r.tech)
      : r.civic !== undefined ? research.civics.includes(r.civic) : false;
    if (has) u.buildings.add(r.building);
    else u.buildings.delete(r.building);
  }
  return u;
}
...
export function computeUnlocks(state: GameState, seat: number): Unlocks {
  ...
  return computeUnlocksIn(
    ...
    s ? getModifiers(state, seat).buildingPrereq : []);
}
...
export interface Modifiers {
   ...
   *  Age. Read by the yield rows that pay only then (the Tsikhe's Faith). */
  goldenAge: boolean;
  ...
  unitPopCost: readonly UnitPopCostRow[];
  ...
  parkAppeal: number;
  /** CIV6 (Cree): how far from one of this seat's CITIES an unclaimed tile
   *  a Trader steps onto comes under its control; 0 = never. */
  tradeGainTileRadius: number;
  ...
  postKillHeal: number;
  /** the class bits a beaten unit may be CAPTURED under (both chassis), 0 = never */
  captureMask: number;
...
export function getModifiers(state: GameState, seat: number): Modifiers {
  ...
  mods.goldenAge = (seatOf(state, seat)?.age ?? 1) === AGE_GOLDEN;
  ...
  mods.captureMask = CAPTURE_ROWS.filter((r) => rowIsFor(r, mods.civ, mods.leader))
    .reduce((m, r) => m | r.classes.reduce((mm, c) => mm | (CLASS_BIT[c] ?? 0), 0), 0);
  ...
  mods.unitPopCost = mine(UNIT_POP_COST_ROWS);
  ...
  mods.buildingPrereq = mine(BUILDING_PREREQ_ROWS);
  ...
  mods.campGoody = mine(CAMP_GOODY_ROWS).length > 0;
  mods.featureAppeal = mine(FEATURE_APPEAL_ROWS);
  mods.allianceSharedVis = mine(ALLIANCE_SHARED_VIS_ROWS).length > 0;
  ...
  mods.tradeGainTileRadius = mine(TRADE_GAIN_TILE_ROWS).reduce((n, r) => Math.max(n, r.radius), 0);
