"""B-25 (Round B3, Slice W) space-race / science-victory self-test.

The GPU space-race chain is GATE-UNREACHABLE at 250t: the scripted player
never queues projects and the rival greedy pick always resolves to a base
project (RESEARCH_GRANTS wins the Campus slot ahead of the space rows), so
no civ starts the chain under the scripted policy. These pokes pin the
semantics the parity rollout can't reach — mirroring tests/space-victory.ts
against the GPU tensors (the occupancy_test / government_test pattern: load
rules + a fixture, drive the GPU BatchSim, assert on its internal state).

Proven here, turn-exact with the TS contract (data/projects.ts +
game.ts/rivals.ts completeProject + the endTurn victoryType recompute):
  * the exported chain: 6 space rows, chain order via rp, single victory
    step, every step tech-gated (rt);
  * a RIVAL completing the victory step -> victoryType 4 (player DEFEAT,
    the domination-defeat mirror) + game_over + space_done bookkeeping,
    through the real rival projects path;
...
def main() -> None:
    ...
    # space rows sit LAST (chain order) so the rival greedy pick never reaches
    ...
    # --- 3) a RIVAL completes the victory step -> victoryType 4 (DEFEAT) ---
    #   Force a live rival capital to hold the EXOPLANET_EXPEDITION project at
    #   full progress, then run the rival phase: the completion path must set
    #   space_done (civ r+1, last step), victoryType 4, and game_over. This is
    #   the rivals.ts completeProject twin (rc.queue = [EXOPLANET...] in the
    #   TS poke). Rivals never SELECT a space row, so we plant it directly.
    assert sim.R >= 1, "need at least one rival for the defeat path"
    sim2 = BatchSim([load_fixture(paths[0])], rules, device="cpu", dtype=torch.float64)
    r, j = 0, 0
    assert bool(sim2.r_alive[0, r]) and bool(sim2.rc_alive[0, r, j]), "rival capital must be alive at turn 0"
    ...
    sim2._rival_phase()
    assert bool(sim2.space_done[0, r + 1, last_step]), "rival's victory step must land in space_done"
    assert int(sim2.victory_type[0]) == 4, f"a rival completing the race is victoryType 4, got {int(sim2.victory_type[0])}"
    assert bool(sim2.game_over[0]), "the game must be over on a rival science win"
    ...
    print("space_race_test OK — chain export/gating/sequence, rival victoryType 4, endTurn 3/4 preservation, _MUTABLE round-trip")
