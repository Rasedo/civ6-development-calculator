"""AUDIT #78 — WATER MILL poke lane.

    python gpu/watermill_test.py

The Civilopedia rule: "Bonus resources improved by Farms gain +1 Food each."

WHY THIS LANE EXISTS. Gate coverage for this mechanic is MEASURED and thin —
1 of 11 alive cities holds a Water Mill and only 4 eligible tiles are owned
across 3 seeds — so scripted parity agreeing is weak evidence rather than
proof. This constructs the configuration directly instead of hoping a seed
wanders into it.
...
def main() -> None:
    ...
    own0 = sim.owner[0] == c0
    body = own0 & (sim.district[0] < 0) & (sim.built_wonder[0] < 0) & sim.workable[0]
    n_elig = int(body.sum())
    ...
    # a future re-source of the building's yields cannot silently invalidate the
    # lane. (Getting this wrong is what the lane caught on its first run.)
