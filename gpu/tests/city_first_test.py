...
def clear_centre(sim, ctr: int) -> None:
    """Despawn whatever the 40-turn evolution parked on the centre — rivals
    garrison their own capitals, so the tile is NOT empty by default (the
    old test placed its 'lone' civilian NEXT TO an incumbent garrison and
    passed only while the garrison+civilian case silently no-opped)."""
    for plane in ("occ_mil", "occ_civ"):
        occ = int(getattr(sim, plane)[0, ctr])
        if occ >= 0:
            lo_v = sim.POOL_LO["v"]
            assert occ >= lo_v, "incumbent should be a rival-pool unit here"
...
def run(civilian: bool, military: bool = False) -> tuple[int, int, bool]:
    ...
    player melee. `military=True` adds a military garrison ALONGSIDE the
    civilian — the seed-9144 t40 class."""
    sim = build()
    ...
    return hp0 - int(sim.rc_hp[0, r, j]), (g_hp0 if dead else g_hp0 - int(sim.v_hp[0, g])), not dead
...
def main() -> None:
    ...
    # seed 9144 rng 2026006111 t40: garrison AND civilian on the centre — the
    # military puts the CITY first (TS cityFirst = enemies==0 || garrisoned);
    # the civilian shields nothing and survives. Before the city_first fix
    # this combination fell through EVERY arm as a silent no-op.
