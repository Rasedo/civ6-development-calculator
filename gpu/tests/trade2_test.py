"""B-23 (Round B8, Slice T) trade-fidelity self-test.

International routes + route duration are largely GATE-REACHABLE on the RIVAL
seat (the scripted parity rollout exercises expiry and the domestic/CS
re-pick every game), but the international leg only fires when a rival
...
Proven here, turn-exact with the TS contract (core/trade.ts
routeYieldsInternational + TRADE_ROUTE_DURATION, rivals.ts rival pick +
income + expiry):
  * the exported constants: trade.intlGold = 3, trade.duration = 20;
  * r_route_dest / r_route_exp are _MUTABLE, long, [B, R, K];
  * an international rival route pays intlGold + dest completed-specialty
    count to GOLD only, and is suspended while at war with the player;
  * duration expiry drops a due route (exp <= turn) and keeps a future one;
  * a "dest gone" international route (its player city center is no longer a
    player center) is dropped;
  * the new tensors ride snapshot/restore.
"""
...
def main() -> None:
    ...
    # --- 1) exported constants mirror core/trade.ts ------------------------
    tr = rj["trade"]
    ...
    assert "r_route_dest" in _MUTABLE and "r_route_exp" in _MUTABLE, "new route tensors must be _MUTABLE"
    B, R = sim.B, sim.R
    ...
    #   Plant a rival-capital route to the player capital's center tile. At t0
    #   the player capital holds no specialty district, so gold = intlGold(3).
    assert sim.R >= 1 and bool(sim.rc_alive[0, 0, 0]), "need a live rival capital"
    dest_tile = int(sim.site[0, 0])  # player capital center tile
    assert int(sim.center_at[0, dest_tile]) == 0 and bool(sim.alive[0, 0]), "player capital must own its center"
    sim.r_routes[0, 0, 0, 0] = int(sim.rc_id[0, 0, 0])  # origin = rival capital
    ...
    # --- 4) dest-gone: intl route whose player center is gone is dropped ----
    s2 = BatchSim([load_fixture(paths[0])], rules, device="cpu", dtype=torch.float64)
    empty_tile = int((s2.center_at[0] < 0).nonzero(as_tuple=True)[0][0])  # not a player center
