"""A civ must not attack a unit it is AT PEACE with.
    ...
    python gpu/peace_target_test.py

WHY THIS EXISTS. `_hostile_vs_unit` and `_hostile_ranged_strike` read the
player occupancy maps UNCONDITIONALLY when the attacker was a rival, while
the rival war act fires whenever that rival is at war with ANYONE (A-19 made
rival-vs-rival wars real). So a rival at war only with ANOTHER RIVAL could
melee or bombard a PLAYER unit it had a peace treaty with.

TS never did this: `attackTargets` filters every candidate through
`unitsHostile(state, unit, u)` — the player arm included.

The scripted gate never caught it because the configuration (a rival at
peace with the player, at war with a rival, standing adjacent to a player
military) does not occur in the 12 fixture seeds — measured 0 occurrences
across 250 turns while 345 turn/rival cases had a rival-only war. That is an
explanation for the green gate, NOT a licence: the goal is a faithful Civ 6
reproduction, so a rule that fires rarely still has to be right.

This lane builds the configuration by hand and asserts the attack does not
happen, for both the melee and the ranged path.
"""
...
def place(sim, tile, seat_is_player, hp=100):
    ...
    sim.v_civ[0, slot] = 0
    sim.v_seat[0, slot] = 1  # civOfRival(0)
    sim.v_type[0, slot] = 2
...
def run(ranged: bool) -> None:
    ...
    if ranged:
        ...
        sim._hostile_vs_unit(att, tgt, "rival", v)
    after = int(sim.p_hp[0, p])
    kind = "ranged" if ranged else "melee"
    assert after == before, (
        f"{kind}: a rival AT PEACE with the player damaged a player unit "
