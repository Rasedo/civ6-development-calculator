"""B-21 (ROUND B8 slice K) city-state envoy/suzerain bonus self-test.

    npm run gpu:export        # (once) writes gpu/fixtures/
    python gpu/cs_bonus_test.py

Scripted parity is the primary correctness bar; these pokes cover the B-21
re-key surface that the 24-seed gate reaches only partially (the player never
exceeds ~5 envoys in-gate, so the 6-envoy tier-2 building lane and the
suzerain contest edges are exercised HERE):

  1. Catalog: _cs_b1idx / _cs_b2idx map each CS type to its tier-1 / tier-2
     BUILDING catalog index (csEnvoyBonuses re-key), _cs_suz_amt == 3, and the
     per-CS suzerain channel (cs_suz_key) round-trips from the fixture.
  2. Player envoy BUILDING bonus: a city holding the CS type's tier-1 building
  ...
  3. Player suzerain perk: a shipped-channel CS the player is STRICTLY suzerain
     of adds +suzerainYield to the CAPITAL in its channel; a descoped CS
     (cs_suz_key = -1) adds nothing; losing the contest (a rival with more
     envoys) removes the perk.
  4. Rival mirror: the same building bonus + suzerain perk on the rival yield
     path (_rival_city_yields_all), off that rival's envoy counts.
...
def test_player_building_bonus(rules, path) -> None:
    ...
    dbonus = float(rules.cs["districtBonus"])
...
def test_player_building_pillage(rules, path) -> None:
    ...
    if sim.S > 1:
        sim.cs_alive[0, 1:] = False
    if not (sim.districts_on and getattr(sim, "_campus_idx", -1) is not None):
...
def test_player_suzerain(rules, path) -> None:
    ...
    if sim.S > 1:
        sim.cs_alive[0, 1:] = False
    # player STRICTLY suzerain of CS0: 4 envoys, rivals at 0
    sim.cs_envoys[0, 0] = 4
    if sim.R > 0:
    ...
    # contest lost: a rival out-envoys the player -> no perk
    if sim.R > 0:
        sim.cs_suz_key[0, 0] = SCIENCE
        sim.cs_r_envoys[0, 0, 0] = 9  # rival 0 dominates
        sim._eff_version += 1
        total, _, _, _ = sim._city_totals(lux=None)
        ...
        print("  player suzerain CONTEST OK: a rival out-envoys the player -> no perk")


def test_rival_bonus(rules, path) -> None:
    sim = BatchSim([load_fixture(path)], rules, device="cpu", dtype=torch.float64)
    for _ in range(20):
        sim.step()
    if sim.R == 0:
        print("  rival test SKIPPED (no rivals)")
        return
    r = 0
    live = (sim.rc_alive[0, r]).nonzero(as_tuple=True)[0]
    if len(live) == 0:
        print("  rival test SKIPPED (rival 0 has no cities)")
        return
    j = int(live[0])
    _force_scientific_cs0(sim)
    if sim.S > 1:
    ...
    def rsci(renvoys: int, suz_key: int = -1) -> float:
        ...
        sim.cs_suz_key[0, 0] = suz_key
        sim._eff_version += 1
        # _rival_city_yields_all returns (food, prod, sci, cul, gold, faith).
        food, prod, sci, cul, gold, faith = sim._rival_city_yields_all(r)
    ...
    assert r3 > r1 + 1e-9, f"rival 3-envoy building bonus did not fire ({r1}->{r3})"
    assert r6 > r3 + 1e-9, f"rival 6-envoy building bonus did not fire ({r3}->{r6})"
    print(f"  rival building bonus OK: science {r1:.2f}(1e) -> {r3:.2f}(3e) -> {r6:.2f}(6e)")

    # rival suzerain perk on the CAPITAL: force this rc to be the capital and
    # make the rival strictly suzerain (envoys 4, player + other rivals at 0)
    ...
    assert ship > desc + 1e-9, f"rival suzerain perk did not add to the capital ({desc}->{ship})"
    print(f"  rival suzerain OK: rc capital science shipped {ship:.2f} vs descoped {desc:.2f}")
