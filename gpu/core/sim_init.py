...
class SimInit:
    ...
    def __init__(self, fixtures: list[dict], rules: Rules, device: str = "cpu", dtype=torch.float64):
        ...
        # for CIV6's Free Cities player (`FREE_SEAT` on its tiles). The war
        # planes add the barbarian row after it (`_alloc_war`).
        ...
        for _k, _dt, _rf, _ex in (
            ("alive", torch.bool, False, None),
            ("center", torch.long, 0, None),
            ("pop", torch.long, 0, None),
            ("hp", torch.long, 0, None),
            ("outer_hp", torch.long, 0, None),
            ("last_hit", torch.long, 0, None),
            ("boost_turn", torch.long, 0, None),
            ...
            ("bldg_pillaged", torch.bool, False, max(len(rules.b_cost), 1)),
        ...
        # THE LEVY (`CityState.levySeat` / `levyEnds`): the seat row holding
        # each minor's army and the turn it comes home, -1 while no one does
        self.citystate_levy_seat = torch.full((B, s_pad), -1, dtype=torch.long, device=device)
        self.citystate_levy_ends = torch.full((B, s_pad), -1, dtype=torch.long, device=device)
        ...
        self.city_reactor_age = torch.full(
            (B, self.CITY_ROWS, civ_city_pad), -1, dtype=torch.long, device=device)
        ...
        self.seat_ext = torch.zeros(B, self.NS, dtype=torch.bool, device=device)
        ...
        self.seat_route_dseat = torch.full((B, self.NS, k_routes), -1, dtype=torch.long, device=device)
        ...
        for _pl, _dt in (
            ...
            ("escorted", torch.bool),
            # CIV6 (The Raven King): this unit is a city-state's, held by LEVY
            # — the mark its clauses read (the 75% upgrade discount survives
            # an upgrade); `levy_src` is the minor's seat it returns to
            # (`Unit.leviedFrom`), -1 on every other unit. Both clear when the
            # unit comes home.
            ("levied", torch.bool),
            ("levy_src", torch.long),
        ...
        self._log_diff: bool = False
        self._diff_events: dict[int, list[str]] = {}
        ...
        # CIV6 (Electronics Factory): yields once the owner holds a technology
        self._bvar_tech_y = {(bi, c): (int(v["techYieldTech"]), torch.tensor([float(x) for x in v["techYields"]], dtype=dtype, device=device))
                             for bi, c, v in self._bvar_cols if int(v["techYieldTech"]) >= 0}
        # CIV6 (Marae): Tourism per passable-feature tile of the city, behind a technology
        self._bvar_tour_feat = {(bi, c): (int(v["tourPerFeat"]), int(v["tourPerFeatTech"]))
                                for bi, c, v in self._bvar_cols if int(v["tourPerFeat"]) != 0}
        ...
        # [civ, leaderRow] — CIV6 (EFFECT_ADJUST_PLAYER_SKIP_FREE_CITY_STEP): a
    ...
    def _alloc_war(self, B: int, n_majors: int, s_pad: int, device) -> None:
        # the war planes' rows: the city planes' (majors, the city-state pad,
        # the Free Cities row) and then the barbarians
        self.BARB_ROW = self.CITY_ROWS
        self.NS = self.BARB_ROW + 1
        _row = torch.zeros(FREE_SEAT + 1, dtype=torch.long, device=device)
        for _r in range(n_majors):
            _row[_r] = _r
        for _c in range(s_pad):
            _row[100 + _c] = n_majors + _c
        _row[BARB_SEAT] = self.BARB_ROW
        _row[FREE_SEAT] = self.FREE_ROW
        self._seat_row = _row
