...
class SimSeats:
    # The hostility square's width: one column per SEAT ID (0 .. FREE_SEAT),
    # plus one last column standing for "no seat" (-1). See `_hostile_table`.
    HOST_N = FREE_SEAT + 2
    ...
    def _seat_production_mask(self, row: int, sites: dict | None = None) -> torch.Tensor:
        ...
        ] if self._scaffold else []
        # ...and ONE host read of which of them any game holds: a district the
        # seat has not unlocked lays an all-False column whatever the placement
        # bodies answer, so the whole per-city scan for it is skipped below.
        _d_live = (torch.stack(d_tech).any(dim=1).tolist() if d_tech else [])
        ...
        # the civic each formation TIER needs, PER UNIT TYPE: the roster's row
        # answers for the domain it names (`_form_civic_ok`), the catalog for
        # every other seat and domain
        form_civ = []
        for _fk in (1, 2):
            _land_f, _sea_f = self._form_civic_ok(row, _fk)
            form_civ.append(torch.where(self.unit_naval.unsqueeze(0),
                                        _sea_f.unsqueeze(1), _land_f.unsqueeze(1)))
        # the TIER's own strategic charge — `_trainable_units` asked the chassis'
        # single rate, and a formation pays a multiple of it
        res_f = []
        for _fk in (1, 2):
        ...
        if nW_m:
            ...
            _built = torch.zeros(B, nW_m + 1, dtype=torch.bool, device=dev)
            _built.scatter_(1, torch.where((_bw >= 0) & (_bw < nW_m), _bw,
                                           torch.full_like(_bw, nW_m)),
                            torch.ones_like(_bw, dtype=torch.bool))
            _unl = [self._wonder_unlock_ok(row, wi) for wi in range(nW_m)]
            _okc = torch.stack([ones_b if u is None else u for u in _unl], dim=1) & ~_built[:, :nW_m]
            _live = _okc.any(dim=0).tolist()
            w_okc = [_okc[:, wi] if (_unl[wi] is not None and _live[wi]) else None
                     for wi in range(nW_m)]
        # THE PROJECT chain, split once for the whole row: `_p_seat[pi]` holds
        # every gate of `availableProjects` that names only the SEAT — the
        # unique-civilization row, the one-time space step, the laser and WMD
        # arms, and the civic — and `_p_city[pi]` names which ONE city-keyed arm
        # the same if/elif ladder leaves for the per-city sweep (1 repair,
        # 2 recommission, 3 competition, 0 none). Deciding the ladder here
        # cannot change which arm a row takes: the ladder reads catalog keys,
        # which do not move inside the sweep.
        _p_seat: list[torch.Tensor] = []
        _p_city: list[int] = []
        for _pi, _prw in enumerate(self._proj_rows):
            _g = self._proj_seat_ok(row, _pi)
            _city_arm = 0
            if int(_prw["rep"]):
                _city_arm = 1
            elif int(_prw["one"]):
                _g = _g & self._once_step_ok(row, _pi)
            elif int(_prw["ls"]):
                _g = _g & self._laser_project_ok(row, _pi)
            elif int(_prw["wmd"]):
                _g = _g & self._wmd_project_ok(row, _pi)
            elif int(_prw["rec"]):
                _city_arm = 2
            elif int(_prw["ao"]) >= 0:
                _city_arm = 3
        ...
        for j in range(self.RC):
            ...
            if self._scaffold and any(_d_live):
                ...
                for si, (di, _ut, _uc, plc, _fc) in enumerate(self._scaffold):
                    # The three CHEAP gates first: `&` is associative, so
                    # `tech & free & cap & place` is the same column read in
                    # this order, and a column already dead never pays
                    # `_district_elig` — the most expensive body in the sweep.
                    if not _d_live[si]:
                        continue
            ...
            if self._ma_bidx >= 0 or self._seaport_bidx >= 0:
                ...
                ok_f[:, :self.NU] = base_f & form_civ[0] & res_f[0]
    ...
    def _apply_war_minors(self, row: int, w: torch.Tensor, n_opp: int, n_tgt: int,
        ...
        min_turns = self.rules.seats["warMinTurns"]
    ...
    def _apply_war_column(self, row: int, war: torch.Tensor, kind: torch.Tensor | None = None) -> None:
        ...
        for k, tgt in enumerate(targets[:n_opp]):
            ...
            pcost = sr["peaceGold0"] + sr["peaceGoldSlope"] * wt.to(torch.float64)
            peace = (
    ...
    def _seat_buy_candidates(self, row: int, active: torch.Tensor):
        ...
        reserve6 = float(self.rules.seats["peaceGold0"])
    ...
    def _seat_class_buy_candidate(self, row: int, active: torch.Tensor):
        ...
        bb = best % NB
        price = self._faith_price(row, self._class_faith_cost(row, bb))
    ...
    def _class_faith_cost(self, row: int, bidx: torch.Tensor) -> torch.Tensor:
        """`buildingFaithCost` for a non-worship row — production cost at the
        faith rate. CIV6 (Valletta's suzerain): the three walls come at
        `_valletta_walls_pct` off, and to that suzerain alone."""
        bi = bidx.clamp(min=0)
    ...
    def _seat_naturalist_candidate(self, row: int, active: torch.Tensor):
        ...
        cost = self._faith_price(row, self._unit_faith_cost(row, self._naturalist_idx))
        ok = (
            active
            & alive.any(dim=1)
    ...
    def _seat_rock_band_candidate(self, row: int, active: torch.Tensor):
        ...
        ok = (
            ...
            & alive.any(dim=1)
            & self.civ_civics[:, row, civ_i]
            & self._afford(self.civ_faith[:, row], self._faith_price(row, self._unit_faith_cost(row, self._band_idx)))
        )
        slot = torch.where(ok, spawn, slot)
        return ok, slot
    ...
    def _seat_district_buy_candidate(self, row: int, active: torch.Tensor, via_faith: bool):
        ...
        no = torch.zeros(self.B, dtype=torch.bool, device=self.device)
        if not self._scaffold or not self.n_governors:
    ...
    def _seat_buy_ladder(self, row: int, active: torch.Tensor, army0: torch.Tensor) -> None:
        ...
        if n_kind is not None and n_j is not None and self._naturalist_idx >= 0:
            ...
            n_price = self._faith_price(row, self._unit_faith_cost(row, self._naturalist_idx))
        ...
        if row in self._driven_buy_band and self._band_idx >= 0:
            ...
            b_price = self._faith_price(row, self._unit_faith_cost(row, self._band_idx))
    ...
    def _apply_seat_production(self, row: int, production: torch.Tensor,
        ...
        # The row is RC (24) columns wide however few cities stand on it, and
        # every arm below is `act & …`: a column with no live city, or with no
        ...
        for j in range(min(int(production.shape[1]), self.RC)):
            ...
            if bool(is_f.any()):
                ...
                for _fk in (1, 2):
                    _land_a, _sea_a = self._form_civic_ok(row, _fk)
                    civ_ok_f = torch.where(f_naval, _sea_a, _land_a)
                ...
                _fmult = torch.tensor(self._form_res_mult, device=self.device, dtype=torch.long)[ftier.clamp(max=len(self._form_res_mult) - 1)]
                for _ui, _sl, _c in self._res_slot_units:
                    self._charge_unit_resource(row, is_f & (fui == _ui), _ui, at=j, mult=_fmult)
    ...
    def _competition_project_ok(self, row: int, j: int, pi: int) -> torch.Tensor:
        ...
        k = int(self._proj_rows[pi]["ao"])
        if k < 0:
            return torch.zeros(self.B, dtype=torch.bool, device=self.device)
        ok = (self.comp_kind >= 0) & (self.comp_kind == k)
        cb = int(self._proj_rows[pi]["cb"])
        if cb >= 0:
    ...
    def _escort_rider(self, gslot: torch.Tensor, here: torch.Tensor, dest: torch.Tensor,
        ...
        for _plane in (self.civilian_at, self.support_at, self.embarked_at):
            ...
            wet_ok = (
                ...
                & (r_naval | self._seat_tech(seat, self._sailing_tech))
            )
    ...
    def _air_cover_scan(self, row: int, tile: torch.Tensor):
        ...
        attacks up to 1 hex away from the weapon"; and separately, "the only
        exceptions to this rule are SHIPS with the Anti-Air Strength stat -
        ...
        keys, slots = [], []
        ring = [(tile, 0)] + [(self.neigh[tile][:, k], 1) for k in range(6)]
        for at, dist in ring:
            live_t = at >= 0
            ac = at.clamp(min=0)
        ...
        S = torch.stack(slots, dim=1)
        pick = K.argmax(dim=1)
        has = K.gather(1, pick.unsqueeze(1)).squeeze(1) >= 0
    ...
    def _interceptor_scan(self, a_seat: torch.Tensor, tgt: torch.Tensor):
        """(slot, has, support f64) — `interceptorAgainst`: the PATROLLING
        fighter that answers a sortie at `tgt`, and the support the other
        covering patrols lend it.

        (Patrols) "Aircraft stationed at an air base do not intercept
        attacking aircraft." As the preview measures it: the patrol ON the
        struck tile answers first, wounded or weaker; among the nearest the
        strongest Combat does; ties go to the lower patrolled tile, then to
        the unit order. Each other covering patrol lends +5 x its hp / 100,
        summed over the hit points first, one product over one division, as
        TS writes it."""
        B, dev = self.B, self.device
        neg = torch.full((B,), -1, dtype=torch.long, device=dev)
        ...
        s = self._type_combat[ty]
        U, T = pat.shape[1], self.T
        idx = torch.arange(U, device=dev).unsqueeze(0)
        key = ((((self._intercept_range - d) * 4096 + s) * T + (T - 1 - pc)) * U + (U - 1 - idx))
        key = torch.where(ok, key, torch.full_like(key, -1))
        pick = key.argmax(dim=1)
        has = key.gather(1, pick.unsqueeze(1)).squeeze(1) >= 0
        hp_sum = (ok.long() * self.unit_hp).sum(dim=1)
        hp_pick = self.unit_hp.gather(1, pick.unsqueeze(1)).squeeze(1)
        support = ((self._intercept_support_cs * (hp_sum - hp_pick)).to(torch.float64)
                   / int(self.rules.combat["unitHp"]))
        return (torch.where(has, pick, neg), has,
                torch.where(has, support, torch.zeros_like(support)))

    def _air_plane_terms(self, plane_seat: torch.Tensor, foe_seat: torch.Tensor) -> torch.Tensor:
        """[B] f64 — `airPlaneTerms`: the seat-pair terms the sortie's plane
        carries into each air sub-combat, the emergencies' net difference and
        the Military alliance's +5 (both `REQUIREMENT_COMBAT_UNIT_VS_UNIT`;
        the lab read the alliance's once per sub-combat), summed first as TS
        sums them."""
        return (self._emergency_pair_cs(plane_seat, foe_seat)
                + self._ally_war_cs(plane_seat, foe_seat).to(torch.float64))

    def _air_cover_answer(self, fire: torch.Tensor, atk_kind: str, u: int, tgt: torch.Tensor,
                          a_slot: torch.Tensor) -> None:
        """the ANTI-AIR BURST a sortie takes, rolled against the AIRCRAFT,
        which is the defender of this one-sided roll — `airCoverAnswer`: the
        weapon's strength against an aircraft against the plane's Ranged
        Strength, each side with its seat-pair terms. The weapon asks the
        CLASS_ANTI_AIR rows of the plane."""
        ...
        c_tile = self.unit_tile.gather(1, cs0.unsqueeze(1)).squeeze(1)
        c_aa = self._anti_air_at(c_type, c_seat)
        c_cs = torch.where(c_aa > 0, c_aa, self._type_combat[c_type])
        c_cs = c_cs + self._air_defense_cs(c_aa, c_seat, c_tile)
        ...
        c_e = c_e + self._promo_cs(
            ...
            attacking=~_t, ranged=_t, vs_air=_t, foe_type=at0, tile=c_tile.clamp(min=0),
        ).to(c_e.dtype)
        c_e = c_e + self._ally_war_cs(c_seat, _seat_p[:, u]).to(c_e.dtype)
        air_d = a_base + self._promo_cs(
            at0, a_promos, attacking=~_t, vs_anti_air=_t, foe_type=c_type,
            tile=_tile_p[:, u]).to(a_base.dtype)
        air_d = air_d + self._air_plane_terms(_seat_p[:, u], c_seat).to(air_d.dtype)
        a_dmg = self._damage_roll(fire, c_e - air_d, k="airc", tile=tgt)
        _hp_p[:, u] = torch.where(fire, _hp_p[:, u] - a_dmg, _hp_p[:, u])

    def _intercept_fight(self, fire: torch.Tensor, atk_kind: str, u: int, tgt: torch.Tensor,
                         i_slot: torch.Tensor, support: torch.Tensor) -> None:
        ...
        at0 = _type_p[:, u].clamp(min=0, max=self.NU - 1)
        a_promos = self._promo_pool(atk_kind)[0][:, u]
        is0 = i_slot.clamp(min=0)
        i_type = self.unit_type.gather(1, is0.unsqueeze(1)).squeeze(1).clamp(min=0, max=self.NU - 1)
        i_seat = self.unit_seat.gather(1, is0.unsqueeze(1)).squeeze(1)
        i_hp = self.unit_hp.gather(1, is0.unsqueeze(1)).squeeze(1)
        i_at = self.unit_patrol.gather(1, is0.unsqueeze(1)).squeeze(1)
        i_e = self._type_combat[i_type] - self._wound(i_hp, i_type) + support
        i_e = i_e + self._promo_cs(
            i_type, self.unit_promos.gather(1, is0.unsqueeze(1)).squeeze(1),
            attacking=~_t, ranged=_t, vs_air=_t, foe_type=at0, tile=i_at.clamp(min=0),
        ).to(i_e.dtype)
        i_e = i_e + self._ally_war_cs(i_seat, _seat_p[:, u]).to(i_e.dtype)
        a_e = self._type_combat[at0] - self._wound(_hp_p[:, u], at0)
    ...
    def _air_strike(self, att: torch.Tensor, tgt: torch.Tensor, atk_kind: str,
        ...
        atk_e = atk_e + self._air_plane_terms(a_seat, d_seat).to(atk_e.dtype)
    ...
    def _charge_unit_resource(self, row: int, hit: torch.Tensor, u_idx: int,
                              at: int | None = None,
                              mult: torch.Tensor | None = None) -> None:
    ...
    def _condemn_heretic(self, row: int, live: torch.Tensor, tile: torch.Tensor,
        ...
        """`condemnHeretic` — a military unit kills the religious unit on its
        own tile.
    ...
    def _cs_resolve_suzerain(self) -> None:
        ...
        if self.S == 0:
            return
        suz_min = int(self.rules.citystate["suzerainEnvoys"])
    ...
    def _unit_near(self, tiles: torch.Tensor, hi: int, *, lo: int = 0,
        ...
        """[B] bool — is there a LIVING major unit whose distance from `tiles`
        lies in [lo, hi]? `lo=1` is TS's `neighbors(...)`, which never includes
        the tile itself. `seat` + `same_seat` picks this seat's own units or
        the ones hostile to it; `types` matches the same chassis; `mask` is a
        per-TYPE filter over the roster."""
        d = self.pair_dist[tiles.clamp(min=0)]                        # [B, T]
        dist = d.gather(1, self.major_unit_tile.clamp(min=0))         # [B, U]
        hit = self.major_unit_alive & (dist <= hi) & (dist >= lo) & (self.major_unit_hp > 0)
        if seat is not None:
            if same_seat:
                hit = hit & (self.major_unit_seat == seat.unsqueeze(1))
            else:
                hit = hit & self._seats_hostile(seat, self.major_unit_seat)
    ...
    def _unit_kill_event(self, killer, vict_type: torch.Tensor, vict_barb: torch.Tensor,
        ...
        # unit's base Combat Strength." (Garde Imperiale): "+10 Great General
        # points for kills." Both are the KILLER chassis's own clause, paid on
        # any victim, and both stand ABOVE the post-combat rows exactly as TS
        # composes them.
    ...
    def _pay_emergency(self, k: int, m: torch.Tensor, members_won: bool) -> None:
        ...
        is_mil = self.emg_kind[:, k] == self._emg_at["MILITARY"]
    ...
    def _city_terrain_count(self, row: int, plane: torch.Tensor) -> torch.Tensor:
        """[B, RC] long — how many tiles of each city carry this ground, the
        `cityMountainCount` twin."""
        sl = self.city_slot_at(row)
    ...
    def _roster_loyalty_aura(self, row: int) -> torch.Tensor:
        """[B, RC] f64 — CIV6 (Toqui, EFFECT_ADJUST_GOVERNOR_IDENTITY_PRESSURE):
    ...
    def _congress_pref(self, r: int, row: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if name == "DEFORESTATION_TREATY":
            ...
            mine = (self.tile_seat == int(self._ROW_SEAT[row])) & ~self.feat_stripped
        ...
        if name == "LUXURY_POLICY":
            ...
            if self._n_lux > 0:
                mine = ((self.lux_id >= 0) & (self.tile_seat == int(self._ROW_SEAT[row]))
    ...
    def _competition_podium(self, done: torch.Tensor) -> None:
        ...
        for b in done.nonzero(as_tuple=True)[0].tolist():
            ...
            for rank, r in enumerate(field):
                ...
                _nb = (int(row["silverBoosts"]) if rank < silver
                       else int(row["bronzeBoosts"]) if rank < bronze else 0)
                if _nb > 0:
                    _one = torch.zeros(self.B, dtype=torch.bool, device=self.device)
                    _one[b] = True
                    self._boost_random_civics(r, _one, _nb, int(row["boostLo"]),
                                              int(row["boostHi"]))
    ...
    def _seat_civics(self, row: int) -> torch.Tensor:
        """[B, nC] the civics a city ROW's holder has researched — a major's
        own record, a minor's own (`citystate_civics`), none for the Free row.
        The yield walk reads research through this and `_seat_techs` alone,
        which is what lets a city-state's city ride the same walk."""
        if row < self.n_majors:
            return self.civ_civics[:, row]
        s = row - self._CITY_MINOR0
        if 0 <= s < self.S:
            return self.citystate_civics[:, s]
        return torch.zeros(self.B, self.civ_civics.shape[2], dtype=torch.bool, device=self.device)
    ...
    def _seat_techs(self, row: int) -> torch.Tensor:
        ...
        s = row - self._CITY_MINOR0
        if 0 <= s < self.S:
            return self.citystate_techs[:, s]
        return torch.zeros(self.B, self.civ_techs.shape[2], dtype=torch.bool, device=self.device)

    def _seat_policies(self, row: int) -> torch.Tensor:
        """[B, nPol] the cards a city ROW's holder has slotted — only a major
        keeps a store; a minor and the Free row slot nothing."""
        if row < self.n_majors:
    ...
    def _imp_adjacency(self, row: int) -> torch.Tensor | None:
        ...
        live = (self.improvement >= 0) & ~self.pillaged
        out = None
        cv = self._seat_civics(row)
        tv = self._seat_techs(row)
        nb = self.neigh
        nbc = nb.clamp(min=0)
        on = (nb >= 0).unsqueeze(0)
    ...
    def _seat_tile_add(self, row: int) -> torch.Tensor:
        ...
        if not self._seat_has_beliefs(row):
            ...
            # ONE live mask for all six generic adds — it is the same [B, T, 1]
            # every term asked for, and building it per term rebuilt three
            # planes and a dtype cast six times over.
            _live = self._tile_add_live()
            if suz is not None:
                plane = plane + suz * _live
            if polY is not None:
                plane = plane + self._imp_gather(polY) * _live
            riv = self._imp_river_plane()
            if riv is not None:
                plane = plane + riv * _live
            fy = self._imp_feat_plane()
            if fy is not None:
                plane = plane + fy * _live
            py = self._plot_yield_plane(row)
            if py is not None:
                plane = plane + py * _live
            uq = self._imp_unique_plane(row)
            if uq is not None:
                plane = plane + uq * _live
    ...
    def _bldg_dark(self, dt_reg: torch.Tensor, bldg_pil: torch.Tensor | None = None) -> torch.Tensor:
        ...
        (`city_bldg_pillaged` at the same index), return [..., NB] bool =
        building b is DARK: its district is COMPLETE-but-PILLAGED, or the
    ...
    def _building_pillaged(self, row: int, sl: slice | None = None) -> torch.Tensor:
        ...
        (`buildingPillaged`'s twin over a column slice)."""
        return self.city_bldg_pillaged[:, row, slice(0, self.RC) if sl is None else sl]
    ...
    def _stockpile_cap(self, row: int) -> torch.Tensor:
        ...
        stand = self.city_bldg[:, row, :cols] & ~self._bldg_dark(self.city_dist_tile[:, row, :cols], self.city_bldg_pillaged[:, row, :cols])
    ...
    def _transfer_city(self, b: int, src_row: int, src_col: int, dst_row: int, *, conquest: bool) -> bool:
        ...
        dev = self.device
        half_hp = (int(self.rules.combat["cityMaxHp"]) + 1) // 2  # Math.round(CITY_MAX_HP / 2)
        ...
        if conquest and int(self.city_alive[b, dst_row].sum()) >= int(self.rules.seats["maxCities"]):
        ...
        # affected one.
        _mil_kind = self._emg_at["MILITARY"]
    ...
    def _found_city_at(self, row: int, want: torch.Tensor, tile: torch.Tensor) -> torch.Tensor:
        ...
        found = (
            ...
            & (alive_row.sum(dim=1) < int(self.rules.seats["maxCities"]))
            # CIV6 (Isolationism): "Can't train or buy Settlers nor settle new
    ...
    def _melee_coloc(self, ok: torch.Tensor, tgt: torch.Tensor, atk_kind: str, u: int, seat: int) -> None:
        """`meleeAttackInner`'s MOVE arm: a melee order onto a tile whose only
        hostile occupant is a RELIGIOUS unit ashore is a step onto it — the
        attacker shares the tile and nothing happens to the religious unit
        ...
        dirs = (self.neigh[here.clamp(min=0)] == tgt.unsqueeze(1)).long().argmax(dim=1)
        self._step_verb(ok, gslot, here, tgt, dirs, seat, torch.zeros_like(ok),
                        clear_camp=POOL_CLASS[atk_kind] == "major")
    ...
    def _hostile_vs_unit(self, att: torch.Tensor, tgt: torch.Tensor, atk_kind: str, u: int) -> None:
        ...
        # civilian OR support, a RELIGIOUS unit never: onto its tile a melee
        # order is a move (`_melee_coloc`)
        cslot_raw = self._melee_class_at(ttc)
        ...
        ok_c = self._melee_ok(cslot_raw, ok_c)
        ...
        if bool(mil_att.any()):
            ...
            atk_e = (atk_cs_all - self._wound(a_hp[:, u], a_type[:, u])
            ...
            # CIV6 (Impi): "+100% Flanking bonus" — the chassis's own
            # multiplier, beside the promotion's.
            atk_e = atk_e + (FLANKING_CS * self._promo_mult(a_type[:, u], a_promos, "FLANK_MULT")
                             * self._type_flank_mult[a_type[:, u].clamp(min=0, max=self.NU - 1)].long() * _fl
                             * (1 + torch.where(
    ...
    def _do_concert(self, row: int, mask: torch.Tensor, tile: torch.Tensor, slot: torch.Tensor) -> None:
        ...
        share = self._gov_mods(row)[12]["concert"].double()
    ...
    def _do_park(self, row: int, mask: torch.Tensor, tile: torch.Tensor, slot: torch.Tensor) -> None:
        ...
        if _dr.numel():
            ...
            self._occ_clear(_dr, tile.clamp(min=0)[_dr], slot[rows][_dies])
        self._eff_version += 1
    ...
    def _centre_strength(self, hrow: torch.Tensor, hcol: torch.Tensor,
        ...
        """[B] f64 — `centreStrength`, A CITY CENTRE'S STANDING STRENGTH, one
        ...
        (`_minor_envoys_received`). The garrison term is fractional, so the
        strength is float64, summed in TS's order: the integer terms first,
        then the garrison, then the envoys."""
        bidx = self._bidx
        ...
        if garrisoned:
            cs = cs + self._garrison_cs(hrow, hcol, seat, base)
        if self.S > 0:
    ...
    def _hostile_table(self) -> torch.Tensor:
        """[B, HOST_N, HOST_N] — `_seats_hostile` for EVERY (acting seat, other
        seat) pair at once, the last index standing for "no seat" (-1).

        The answer depends on the war matrix and the fixed seat classes and on
        nothing else, so the twelve thousand per-unit-per-verb probes a turn
        become one build per DECLARATION plus a gather each. The body below is
        the tensor arm of `_seats_hostile`, evaluated over the whole seat
        square: a seat is never hostile to itself, a Free City is hostile to
        every other seat, a barbarian to every non-barbarian, and otherwise the
        war matrix decides.
        ...
        a_b, b_b = barb.unsqueeze(1), barb.unsqueeze(0)
        a_f, b_f = free.unsqueeze(1), free.unsqueeze(0)
    ...
    def _seats_hostile(self, a_seat, b_plane: torch.Tensor) -> torch.Tensor:
        # A seat is never hostile to ITSELF, stated explicitly in the table:
    ...
    def _step_verb(
        ...
        if bool(transition.any()):
            ...
            # reusing it on the disembark charged a crossing TS never
            # charges — `moveCostInto` is terrain and road ALONE, and
            # `riverCharge` rides only the non-transition arm (`stepUnit`).
            trans_step = torch.where(
                to_water, torch.full_like(land_cost, self._mp_scale),
                self._mp_scale + terr)
            w_end = torch.where(to_water, dc1.squeeze(1), hc)
            ...
            # NOBODY crosses the shore this step, so the whole transition
            # arm is dead weight: `torch.where(transition, …, base_step)`
            # is `base_step` term for term when `transition` is empty.
            cost = base_step
        ...
        if bool(major.any()):
            ...
            see_thr = self._sees_through(u_type, u_promos)
            ...
            # WIDEST of the formation's members. With no rider every term is
            # gated on `_rr >= 0` and leaves both answers untouched.
            for _ri in range(rider.shape[1] if _riders else 0):
                _rr = rider[:, _ri]
                _rc = _rr.clamp(min=0)
    ...
    def _in_enemy_zoc(self, dest: torch.Tensor, seat, mover_type: torch.Tensor) -> torch.Tensor:
        ...
        The zone is read AT THE SIX NEIGHBOURS the halt asks about, never built
        over the whole map: every term below is a function of the tile alone,
        so gathering the ring first and asking the predicate there is the same
        answer over 6 columns instead of T."""
        dc = dest.clamp(min=0)
        dn = self.neigh[dc]                                   # [B, 6]
        dnc = dn.clamp(min=0)
        mil = self.military_at.gather(1, dnc)
        here = mil >= 0
        ...
        halt = ((dn >= 0) & (riv == 0) & hostmil).any(dim=1)
        # CIV6 (ABILITY_IGNORE_ZOC): light, heavy and ranged cavalry — a chariot
        # class is not on the list unless it carries the ability itself. The
        # three catalog vectors are fixed at load, so the OR is built once.
        ign = getattr(self, "_zoc_ign_cache", None)
        if ign is None:
    ...
    def _attack_encampment(self, att: torch.Tensor, tile: torch.Tensor, atk_kind: str, u: int) -> None:
        ...
        a_promos = self._promo_pool(atk_kind)[0][:, u]
        atk_e = (atk_cs - self._wound(a_hp[:, u], a_type[:, u])
    ...
    def _conquistador_convert(self, b: int, captor_row: int, ctr: int) -> None:
    ...
    def _melee_city(self, att: torch.Tensor, tgt: torch.Tensor, atk_kind: str, u: int) -> None:
        ...
        self.city_hp[fell, hr, sl] = round(int(self.rules.combat["cityMaxHp"]) / 2)
    ...
    def _hostile_ranged_strike(self, att: torch.Tensor, tgt: torch.Tensor, atk_kind: str, u: int,
        ...
        if bool(city_att.any()):
            ...
            if not barb:
                ...
                atk_e = atk_e + self._rel_atk_cs(a_seat, tgt).to(atk_e.dtype)
        ...
        # EACH ARM BELOW IS A FALLBACK: a row the arm above it took can never
        # fire here, so a shot the CITY arm resolved leaves every mask under it
        # empty and its scan — the district block, the [B, T] city-state
        # scatter, the target fold — has nothing to answer about.
        _rest = att & ~city_att
        ...
        if bool(cs_att.any()):
            ...
            atk_cs = (self._city_ranged_strength(ut0, a_seat, cs_outer)
                      ...
                      + self._assault_promo_cs(ut0, a_promos, a_tile, ranged=True))
            if not barb:
                atk_cs = atk_cs + self._rel_atk_cs(a_seat, tgt).to(atk_cs.dtype)
        ...
        if bool(_rest.any()):
            ...
            if bool(unit_att.any()):
                ...
                # the chassis' own ranged strength plus the four pool adders —
                # assembled HERE because the city and city-state arms assemble
                # their own (off `_city_ranged_strength`), so a shot that never
                # reaches a unit paid for four helper calls it could not use.
                atk_rs = (self._type_ranged_strength[ut0] + self._form_cs_pool(atk_kind, u)
                          + self._convoy_cs_pool(atk_kind, u) - self._fuel_short_cs_pool(atk_kind, u) + self._chassis_atk_cs_pool(atk_kind, u))
                atk_e = atk_rs - self._wound(a_hp, ut0) + self._promo_cs(
    ...
    def _ranged_attack(self, att: torch.Tensor, tgt: torch.Tensor, atk_kind: str,
        ...
        atk_base = (atk_rs0 + self._form_cs_pool(atk_kind, u)
                    + self._convoy_cs_pool(atk_kind, u) - self._fuel_short_cs_pool(atk_kind, u) + self._chassis_atk_cs_pool(atk_kind, u) - self._wound(a_hp[:, u], a_type[:, u]))
        ...
        rel_city = self._rel_atk_cs(aseat, tgt).to(atk_base.dtype)
        # both city arms split the roll by the attacker's hit class
    ...
    def _seat_quest_phase(self, row: int, active: torch.Tensor) -> None:
        ...
        B, S, dev = self.B, self.S, self.device
        rr = self.rules.citystate
        cooldown = int(rr["questCooldown"])
        q_env = int(rr["questEnvoys"])
        csc = self.citystate_center[:, :S].clamp(min=0)
        met_live = self.seat_citystate_met[:, row, :S] & self.citystate_alive[:, :S]
    ...
    def _trade_capacity(self, row: int) -> torch.Tensor:
        ...
        if S > 0:
            trade_ti = int(self.rules.citystate["tradeIdx"])
