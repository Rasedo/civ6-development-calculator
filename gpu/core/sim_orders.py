...
class SimOrders:
    def _apply_seat_unit_actions(self, r: int, actions: torch.Tensor) -> None:
        ...
        for row in range(simbase.SEAT0_POOL_MAX):
            slot = smap[:, row]
            present = (slot >= 0) & ctl
            ...
            if not bool(act.any()):
                continue
            here = self.civ_unit_tile.gather(1, sc.unsqueeze(1)).squeeze(1)
            is_civ = self._type_charges[self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1)] > 0
            ...
            if getattr(self, "_A_FOUND", -1) >= 0 and self._settler_idx >= 0:
                ...
                if bool(fnd.any()):
                    ...
                    if bool(made_f.any()):
                        ...
                        self.civ_unit_alive[rows_f, sc[rows_f]] = False
            # --- moves 0-5 ---
            mv = act & (a < 6)
            if bool(mv.any()):
                ...
                if self._embark_live:
                    ...
                    _vt_mv2 = self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1).clamp(min=0, max=self.NU - 1)
                ...
                _mp_d = self.civ_unit_mp.gather(1, sc.unsqueeze(1)).squeeze(1)
            ...
            # targets) ---
            atk = act & (a >= 6) & (a < 12) & ~is_civ & ~self.civ_unit_emb.gather(1, sc.unsqueeze(1)).squeeze(1)  # embarked cannot attack (the war act's ~civ_unit_emb)
            if bool(atk.any()):
                ...
                if bool(valid_t.any()):
                    ...
                    _mt = self.military_at.gather(1, tc.unsqueeze(1)).squeeze(1)
                    _ct = self.civilian_at.gather(1, tc.unsqueeze(1)).squeeze(1)
                    ...
                    _vt_d2 = self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1).clamp(min=0, max=self.NU - 1)
                    _melee_d2 = self._type_ranged_strength[_vt_d2] <= 0
                    ...
                    if bool(_rngd_att.any()):
                        for b_ in _rngd_att.nonzero(as_tuple=True)[0].tolist():
                            ...
                            one_[b_] = True
                            _fired_ = self._hostile_ranged_strike(one_, tgt, "civ", v_)
                            ...
                            if bool(_fired_[b_]):
                                self.civ_unit_mp[b_, v_] = 0  # a strike spends the turn (TS movesLeft = 0)
                    ...
                    for b_ in range(B):
                        ...
                        one[b_] = True
                        if bool(unit_att[b_]):
                            self._hostile_vs_unit(one, tgt, "civ", v)
                            self.civ_unit_mp[b_, v] = 0  # the turn is spent (TS movesLeft = 0)
                            ...
                            _csr2 = self._assault_city_state(one, _css, tgt.clamp(min=0), "civ", v)
            ...
            if bool(snp.any()):
                ...
                vt_d = self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1).clamp(min=0, max=self.NU - 1)
                ok_s2 = (
                    snp & (tgt_s >= 0)
                    & (self._type_ranged_strength[vt_d] > 0) & (self._type_ranged_range[vt_d] >= 2)
                    & ~self.civ_unit_emb.gather(1, sc.unsqueeze(1)).squeeze(1)
                )
                if bool(ok_s2.any()):
                    tcs = tgt_s.clamp(min=0)
                    _mt2 = self.military_at.gather(1, tcs.unsqueeze(1)).squeeze(1)
                    _ct2 = self.civilian_at.gather(1, tcs.unsqueeze(1)).squeeze(1)
                    ...
                    for b_ in range(B):
                        ...
                        if bool(u_hit[b_] | c_hit[b_]):
                            ...
                            _fired2 = self._hostile_ranged_strike(one, tgt_s, "civ", v)
                            if bool(_fired2[b_]):
                                self.civ_unit_mp[b_, v] = 0
            ...
            chp = (
                ...
                & (a == self._A_CHOP)
                & is_civ
                & (self.civ_unit_charges.gather(1, sc.unsqueeze(1)).squeeze(1) > 0)
                & (ftr_c > 0)
                & unlocked_c
            ...
            if bool(chp.any()):
                ...
                self.civ_unit_charges[rows_c, sc[rows_c]] -= 1
                self.civ_unit_mp[rows_c, sc[rows_c]] = 0  # the turn is spent (TS movesLeft = 0)
                spent_c = chp & (self.civ_unit_charges.gather(1, sc.unsqueeze(1)).squeeze(1) <= 0)
                if bool(spent_c.any()):
                    dr = spent_c.nonzero(as_tuple=True)[0]
                    self.civ_unit_alive[dr, sc[dr]] = False
                    self.civilian_at[(dr, here[dr])] = -1
            ...
            if bool(bld.any()):
                ...
                if bool(did.any()):
                    ...
                    self.civ_unit_charges[rows_, sc[rows_]] -= 1
                    self.civ_unit_mp[rows_, sc[rows_]] = 0  # the turn is spent (TS movesLeft = 0)
                    self._eff_version += 1
                    spent = did & (self.civ_unit_charges.gather(1, sc.unsqueeze(1)).squeeze(1) <= 0)
                    if bool(spent.any()):
            ...
            if self.improvements_on and self._builder_idx >= 0 and self._act_names:
                ...
                if bool(_rbase.any()):
                    ...
                    if bool(_didr.any()):
                        ...
                        self.civ_unit_charges[_rw, sc[_rw]] -= 1
                        self.civ_unit_mp[_rw, sc[_rw]] = 0  # the turn is spent
                        self._eff_version += 1
                        _sp = _didr & (self.civ_unit_charges.gather(1, sc.unsqueeze(1)).squeeze(1) <= 0)
                        if bool(_sp.any()):
                            _dr = _sp.nonzero(as_tuple=True)[0]
                            self.civ_unit_alive[_dr, sc[_dr]] = False
                            self.civilian_at[(_dr, here[_dr])] = -1
                ...
                _okrp = (
                    ...
                    & (self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1) == self._builder_idx)
                    & (self.civ_at.gather(1, _tcd.unsqueeze(1)).squeeze(1) == r)
                    & (
            ...
            if self._act_names and self._A_PILLAGE > 0:
                ...
                _okpl = (
                    ...
                    & (self._type_combat[self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1).clamp(min=0)] > 0)
                    & _enp & (_hip | _hdp)
                )
                if bool(_okpl.any()):
                    ...
                    _hr = _rw[_pi][_hl]
                    if _hr.numel():
                        self.civ_unit_hp[_hr, sc[_hr]] = torch.minimum(
                            self.civ_unit_hp[_hr, sc[_hr]] + 25,
                            torch.full_like(self.civ_unit_hp[_hr, sc[_hr]], 100),
                        )
                    _pd = ~_pi & _hdp[_rw]
            ...
            if getattr(self, "_A_SPREAD", -1) >= 0:
                ...
                if bool(spx.any()):
                    vt_sp = self.civ_unit_type.gather(1, sc.unsqueeze(1)).squeeze(1).clamp(min=0, max=self.NU - 1)
                    ...
                    if bool(ok_sp.any()):
                        ...
                        if bool(landed.any()):
                            ...
                            self.civ_unit_charges[lr, sc[lr]] -= 1
                            self.civ_unit_mp[lr, sc[lr]] = 0
                            dead_sp = landed & (self.civ_unit_charges.gather(1, sc.unsqueeze(1)).squeeze(1) <= 0)
                            if bool(dead_sp.any()):
    ...
    def _seat0_attack_civ_city(self, att: torch.Tensor, tgt: torch.Tensor, p: int) -> None:
        ...
        aftermath."""
        _r = self._assault_civ_city(att, tgt, "seat0", p)
        if _r is None:
            return
        rows, civ, slot, died, ttc = _r
        if bool(died.any()):
            dr = died.nonzero(as_tuple=True)[0]
            here_d = self.seat0_unit_tile[dr, p]
            self._dig_at(dr, here_d)  # killUnit's dig
            self.military_at[(dr, here_d)] = -1
            self.seat0_unit_alive[dr, p] = False
    ...
    def _apply_unit_actions(self, actions: torch.Tensor) -> None:
        ...
        """
        p_high = int(self.seat0_unit_next.max().item())
        ...
        if p_high:
            live_any = self.seat0_unit_alive[:, :p_high].any(dim=0)
        ...
        for p in p_live:
            ...
            if getattr(self, "_A_FOUND", -1) >= 0 and self._settler_idx >= 0:
                ...
                if bool(fnd.any()):
                    ...
                    if bool(made_f.any()):
                        ...
                        self.seat0_unit_alive[rows_f, p] = False
            ...
            _tm = self.military_at.gather(1, tc.unsqueeze(1)).squeeze(1)
            ...
            vc_slot_t = torch.where((_tcs > 0) & (_tcs != BARB_SEAT), _tc_ - self.POOL_LO["civ"], torch.full_like(_tc_, -1))
            ...
            if self._rl_ranged_active:
                rngd = self._type_ranged_strength[self.seat0_unit_type[:, p]] > 0
            else:
            ...
            citystate_here = (
                ...
                # `meleeAttack`'s cityStateTarget is undefined unless the minor is at
            ...
            civk = alive & (a >= 6) & (a < 12) & (tgt >= 0) & (bslot < 0) & ~v_ok & vc_ok & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & ~rngd
            if bool(civk.any()):
                ...
                ks = vc_slot_t[kr]
                ct = tc[kr]
                self.civ_unit_alive[kr, ks] = False
                nslot = self.seat0_unit_next[kr]
                assert int(nslot.max()) < simbase.SEAT0_POOL_MAX, "p slot pool exhausted — raise simbase.SEAT0_POOL_MAX"
                self.seat0_unit_alive[kr, nslot] = True
                self.seat0_unit_tile[kr, nslot] = ct
                self.seat0_unit_seat[kr, nslot] = 0
                self._carry_capture(kr, ks + self.POOL_LO["civ"], nslot + self.POOL_LO["seat0"])
                self.civilian_at[(kr, ct)] = nslot + self.POOL_LO["seat0"]
                self.seat0_unit_next[kr] += 1
                self._gen_ver += 1  # the captured civilian may be a general (owner flip) → invalidate the aura plane
                self.seat0_unit_mp[:, p] = torch.where(civk, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            ...
            siege = alive & (a >= 6) & (a < 12) & (tgt >= 0) & city_first & civ_city_ok & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & (self._type_ranged_strength[self.seat0_unit_type[:, p]] == 0)
            if bool(siege.any()):
                self._seat0_attack_civ_city(siege, tgt, p)
                self.seat0_unit_mp[:, p] = torch.where(siege, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            ...
            if self._encamp_didx >= 0:
                ...
                enc_att = (
                    ...
                    & ~civ_city_ok
                    & enc_ok
                    & (self._type_combat[self.seat0_unit_type[:, p]] > 0)
                    & (self._type_ranged_strength[self.seat0_unit_type[:, p]] == 0)
                )
                if bool(enc_att.any()):
                    self._attack_encampment(enc_att, tc, "seat0", p)
                    self.seat0_unit_mp[:, p] = torch.where(enc_att, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            att = alive & (a >= 6) & (a < 12) & (tgt >= 0) & garrisoned & ~city_here & (self._type_combat[self.seat0_unit_type[:, p]] > 0)
            ...
            if bool(att.any()):
                is_b = bslot >= 0
                atk_cs = self._type_combat[self.seat0_unit_type[:, p]]
                b_cs = self._type_combat[self.barb_unit_type.gather(1, bslot.clamp(min=0).unsqueeze(1)).squeeze(1)]
                v_cs = self._type_combat[self.civ_unit_type.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)]
                b_fy = self.barb_unit_fortify.gather(1, bslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                v_fy = self.civ_unit_fortify.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                ...
                v_lvl5 = torch.where(is_b, torch.zeros_like(is_b, dtype=torch.long), self._xp_lvl_bonus(self.civ_unit_xp.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)))
                ...
                v_embd = self.civ_unit_emb.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1) & ~is_b
                ...
                b_hp = self.barb_unit_hp.gather(1, bslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                v_hpd = self.civ_unit_hp.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                atk_e = atk_cs - self._wound(self.seat0_unit_hp[:, p]) - 5.0 * self._river_cross(here, tgt) + p_lvl5  # river crossing + attacker veterancy
                ...
                _dsl = torch.where(is_b, bslot + self.POOL_LO["barb"], vslot + self.POOL_LO["civ"])
                rows, def_dead, atk_dead = self._melee_exchange(
                    att, tgt, tc, _dsl, ~is_b, self.seat0_unit_hp, p, atk_e, def_e)
                # ONE battle, scored for both sides — before the advance moves
                # anybody and before the tile can change hands.
                self._ww_battle(att, self._row_of(self.seat0_unit_seat[:, p]),
                ...
                if bool(atk_dead.any()):
                    ...
                    self.seat0_unit_alive[:, p] = self.seat0_unit_alive[:, p] & ~atk_dead
                ...
                if bool(adv.any()):
                    ...
                    self.military_at[(vr, here[vr])] = -1
                    self.seat0_unit_tile[vr, p] = tgt[vr]
                    self.military_at[(vr, tgt[vr])] = p
            ...
            if bool(civ_only_att.any()):
                is_b = bslot >= 0
                atk_rs = self._type_ranged_strength[self.seat0_unit_type[:, p]]
                b_cs = self._type_combat[self.barb_unit_type.gather(1, bslot.clamp(min=0).unsqueeze(1)).squeeze(1)]
                v_cs = self._type_combat[self.civ_unit_type.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)]
                b_fy = self.barb_unit_fortify.gather(1, bslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                v_fy = self.civ_unit_fortify.gather(1, vslot.clamp(min=0).unsqueeze(1)).squeeze(1)
                ...
                if len(rows) > 0:
                    _lo = torch.where(is_b, torch.full_like(bslot, self.POOL_LO["barb"]), torch.full_like(bslot, self.POOL_LO["civ"]))
                ...
                self._ww_battle(civ_only_att, self._row_of(self.seat0_unit_seat[:, p]),
                ...
                if len(surv_rv) > 0:
                    self.unit_xp[surv_rv, vslot[surv_rv] + self.POOL_LO["civ"]] += XP_DEFEND
            # Any fight spends the attacker's MP (att|civ_only_att is the original
            # validated attack set — both branches always execute).
            self.seat0_unit_mp[:, p] = torch.where(att | civ_only_att, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            ...
            civ_only_civ = alive & (a >= 6) & (a < 12) & (tgt >= 0) & (bslot < 0) & ~v_ok & vc_ok & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & rngd
            if bool(civ_only_civ.any()):
                ...
                civ_embd = self.civ_unit_emb.gather(1, vc_slot_t.clamp(min=0).unsqueeze(1)).squeeze(1)
                ...
                civ_hp = self.civ_unit_hp.gather(1, vc_slot_t.clamp(min=0).unsqueeze(1)).squeeze(1)
                atk_e = atk_rs - self._wound(self.seat0_unit_hp[:, p]) + p_lvl5  # attacker veterancy
                ...
                _, _sp = self._flank_support(tgt, vc_civ_t + 1, torch.full_like(tgt, -1))
                ...
                atk_naval = self.unit_naval[self.seat0_unit_type[:, p].clamp(min=0, max=self.NU - 1)] | self.seat0_unit_emb[:, p]
                atk_e = atk_e + self._gen_aura_cs(torch.zeros(self.B, dtype=torch.long, device=self.device), self.seat0_unit_tile[:, p], atk_naval).to(atk_e.dtype)
                ...
                ks = vc_slot_t[rows]
                self.civ_unit_hp[rows, ks] -= d_def[rows]
                dead = self.civ_unit_hp[rows, ks] <= 0
                self._dig_at(rows[dead], tc[rows[dead]])  # killUnit's dig
                self.civ_unit_alive[rows[dead], ks[dead]] = False
                self.civilian_at[(rows[dead], tc[rows[dead]])] = -1
                self._ww_battle(civ_only_civ, self._row_of(self.seat0_unit_seat[:, p]),
                ...
                self.seat0_unit_mp[:, p] = torch.where(civ_only_civ, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)

            # --- melee vs a CITY-STATE CENTER — meleeAttack's cityStateTarget
            ...
            citystate_hit = (
                ...
                & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & ~rngd
            )
            _csr = self._assault_city_state(citystate_hit, citystate_sc, tgt, "seat0", p)
            if _csr is not None:
            ...
            civ_only_sieg = alive & (a >= 6) & (a < 12) & (tgt >= 0) & city_first & civ_city_ok & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & rngd  # rangedAttackInner shares the exact cityFirst rule
            if bool(civ_only_sieg.any()):
                ...
                gslot2 = self.military_at.gather(1, tc.unsqueeze(1)).squeeze(1)
                ...
                atk_e2 = self._type_ranged_strength[self.seat0_unit_type[:, p]] - self._wound(self.seat0_unit_hp[:, p]) + p_lvl5  # wound (the city is not a unit) + veterancy
                ...
                _rngrc_nav = self.unit_naval[self.seat0_unit_type[:, p].clamp(min=0, max=self.NU - 1)] | self.seat0_unit_emb[:, p]
                atk_e2 = atk_e2 + self._gen_aura_cs(
                    torch.zeros(self.B, dtype=torch.long, device=self.device), self.seat0_unit_tile[:, p], _rngrc_nav
                ).to(atk_e2.dtype)
                d_city2 = self._damage_roll(civ_only_sieg, atk_e2 - def_cs2, k="rngrc", tile=tgt)
                self._ww_battle(civ_only_sieg, self._row_of(self.seat0_unit_seat[:, p]),
                ...
                self.seat0_unit_mp[:, p] = torch.where(civ_only_sieg, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            civ_only_cs = (
                alive & (a >= 6) & (a < 12) & (tgt >= 0)
                & city_first & (civ_city_civ_t < 0) & citystate_here  # city-first (garrison does not shield; lone civilian does)
                & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & rngd
            ...
            if bool(civ_only_cs.any()):
                ...
                atk_e3 = self._type_ranged_strength[self.seat0_unit_type[:, p]] - self._wound(self.seat0_unit_hp[:, p]) + p_lvl5  # wound (the CS centre is not a unit) + veterancy
                # Aura inside the ranged-strength parentheses, after
                # xpLevelBonus (rangedAttack's city-state branch).
                atk_naval = self.unit_naval[self.seat0_unit_type[:, p].clamp(min=0, max=self.NU - 1)] | self.seat0_unit_emb[:, p]
                atk_e3 = atk_e3 + self._gen_aura_cs(torch.zeros_like(here), self.seat0_unit_tile[:, p], atk_naval).to(atk_e3.dtype)
                d_cs3 = self._damage_roll(civ_only_cs, atk_e3 - def_cs3, k="rngcs", tile=tgt)
                self._ww_battle(civ_only_cs, self._row_of(self.seat0_unit_seat[:, p]),
                ...
                self.seat0_unit_mp[:, p] = torch.where(civ_only_cs, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
            ...
            self.seat0_unit_xp[:, p] = torch.where(p_attacked, self.seat0_unit_xp[:, p] + XP_ATTACK, self.seat0_unit_xp[:, p])
            ...
            if self._builder_idx >= 0:
                ...
                ok_pl = (a == self._A_PILLAGE) & self.seat0_unit_alive[:, p] & (self._type_combat[self.seat0_unit_type[:, p]] > 0) & _en & (_hi | _hd)
                if bool(ok_pl.any()):
                    _pi = ok_pl & _hi
                    if bool(_pi.any()):
                        ...
                        self.seat0_unit_hp[_r3, p] = torch.where(
                            _heal, (self.seat0_unit_hp[_r3, p] + 25).clamp(max=_cap), self.seat0_unit_hp[_r3, p]
                        )
                    _pd = ok_pl & ~_hi & _hd
                    if bool(_pd.any()):
                        _r4 = _pd.nonzero(as_tuple=True)[0]
                        self.district_pillaged[_r4, hc0[_r4]] = True
                    self.seat0_unit_mp[:, p] = torch.where(ok_pl, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
                    self._eff_version += 1
                ...
                if self.improvements_on and self._builder_idx >= 0:
                    ...
                    _b2 = (
                        self.seat0_unit_alive[:, p]
                        & (self.seat0_unit_type[:, p] == self._builder_idx)
                        & (self.seat0_unit_charges[:, p] > 0)
                    ...
                    for _k in range(3, self._imp_unlock.numel()):
                        ...
                        if bool(_ok2.any()):
                            ...
                            self.seat0_unit_charges[:, p] = torch.where(_ok2, self.seat0_unit_charges[:, p] - 1, self.seat0_unit_charges[:, p])
                            _gone = _ok2 & (self.seat0_unit_charges[:, p] <= 0)
                            if bool(_gone.any()):
                                _g2 = _gone.nonzero(as_tuple=True)[0]
                                self.civilian_at[(_g2, self.seat0_unit_tile[_g2, p])] = -1
                                self.seat0_unit_alive[:, p] = self.seat0_unit_alive[:, p] & ~_gone
                            self._eff_version += 1
                ...
                if bool(ok_rp.any()):
                    rb_ = ok_rp.nonzero(as_tuple=True)[0]
                    tt_ = hc0[rb_]
                    _imp = self.pillaged[rb_, tt_]
                    self.pillaged[rb_[_imp], tt_[_imp]] = False
                    _dis = ~_imp & self.district_pillaged[rb_, tt_]
                    self.district_pillaged[rb_[_dis], tt_[_dis]] = False
                    self.seat0_unit_mp[:, p] = torch.where(ok_rp, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
                    self._eff_version += 1
                ...
                if bool(ok_c.any()):
                    ...
                    tiles_c = hc0[rows_c]
                    self.seat0_unit_mp[:, p] = torch.where(ok_c, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
                    self._strip_feature_at(rows_c, tiles_c)
                    if self.LUMBER >= 0:
                        was_l = self.improvement[rows_c, tiles_c] == self.LUMBER
                        self.improvement[rows_c, tiles_c] = torch.where(was_l, torch.full_like(self.improvement[rows_c, tiles_c], -1), self.improvement[rows_c, tiles_c])
                    ...
                    self.seat0_unit_charges[:, p] = torch.where(ok_c, self.seat0_unit_charges[:, p] - 1, self.seat0_unit_charges[:, p])
                    spent = ok_c & (self.seat0_unit_charges[:, p] <= 0)
                    if bool(spent.any()):
                        dr = spent.nonzero(as_tuple=True)[0]
                        self.civilian_at[(dr, self.seat0_unit_tile[dr, p])] = -1
                        self.seat0_unit_alive[dr, p] = False
            ...
            if self.improvements_on and self._builder_idx >= 0:
                ...
                for act, valid, imp in ((13, farmable, self.FARM), (14, mineable, self.MINE), (15, woodsy, self.LUMBER)):
                    ...
                    if bool(bld.any()):
                        ...
                        self.seat0_unit_charges[rows, p] -= 1
                        self.seat0_unit_mp[:, p] = torch.where(bld, torch.zeros_like(self.seat0_unit_mp[:, p]), self.seat0_unit_mp[:, p])  # the turn is spent (TS movesLeft = 0)
                        self._eff_version += 1
                        gone = bld & (self.seat0_unit_charges[:, p] <= 0)
                        if bool(gone.any()):
                            gr = gone.nonzero(as_tuple=True)[0]
                            self.civilian_at[(gr, here[gr])] = -1
                            self.seat0_unit_alive[:, p] = self.seat0_unit_alive[:, p] & ~gone

            # --- step to a neighbor (0..5) --------------------------------------
            mv = self.seat0_unit_alive[:, p] & (a >= 0) & (a < 6)
            if not bool(mv.any()):
                continue
    ...
    def _barbarian_phase(self) -> None:
        ...
        guard = torch.zeros(B, simbase.POOL_MAX, dtype=torch.bool, device=dev)
        if any_camp:
        ...
        for u in u_live:
            ...
            if bool(vc_att.any()):
                self._attack_civ_city(vc_att, ttc, u)
