"""G-V cliff #2 mechanism: WHY do scripted-player cities decline in the
long game — loyalty flips (peace) or rival conquest (war)?

Per game over 300 turns: track peak player cities, final, and whether the
player was EVER at war with any rival. If cities are lost in games that
never went to war -> loyalty flips. If losses concentrate in at-war games
-> conquest. Also splits the empire-wide city-loss count by war state at
the turn of loss.
"""
import sys, torch
