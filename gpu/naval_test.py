"""#45 / AUDIT B-6 (naval + embarkation) self-test — the gate-UNREACHABLE surfaces.

    npm run gpu:export        # (once) writes gpu/fixtures/  (naval catalog)
    python gpu/naval_test.py

N2 shipped naval LIVE and gate-green, but the 24×250 scripted parity rollout
reaches almost none of the naval combat surface (N2 log: naval-unit COMBAT
0/24, player naval 0/24 — the scripted+controlled player builds no ships and
the rival galley policy rarely fights). These pokes pin those semantics the
same way space_race_test / war_test do: build a BatchSim from a fixture, force
the state in-memory, then drive the EXACT engine twin (_apply_unit_actions,
_barbarian_phase pcstk, _rival_phase rcstk, _spawn_player, _flank_support) and
assert TS-mirroring behaviour.

Covered here (all gate-unreachable):
  1. GALLEY naval melee — batter + CAPTURE a coastal rival city.
  2. GALLEY naval melee — CAPTURE a coastal city-state.
  3. QUADRIREME range-1 bombard — a rival UNIT (no retaliation, no advance).
  4. QUADRIREME range-1 bombard — a rival CITY (HP floors at 1, never captures).
  5. PLAYER naval — spawn on WATER (_spawn_player naval probe) + attack; and the
     documented #50 residual: the RL/controlled MOVE verb cannot step a ship
     onto water (player-move apply reads the land `passable` plane, no wpass).
  6. OCEAN gate — a naval mover's spawn probe is blocked over OCEAN pre-
     ...
     is the TS twin in tests/naval-embark.test.ts.)
  ...
  8. Embarked civilian CAPTURE — POOL-END invariant + keeps-embarked (GPU).
  9. B-7 flank/support — a NAVAL ally counts; the same ally EMBARKED counts 0.
"""
...
def is_center(sim, t: int) -> bool:
    """True iff tile t IS a city / rival-city / city-state CENTER (territory
    tiles around a center are NOT centers)."""
    if int(sim.center_at[0, t]) >= 0 or int(sim.rvcity_at[0, t]) >= 0:
        return True
    s = int(sim.cs_at[0, t])
...
def empty_neighbor(sim, ctr: int) -> int:
    ...
    itself a city/CS/rival-city CENTER (territory is allowed — we stand on it,
    ...
    for d in range(6):
        ...
        if (
            ...
            and int(sim.rv_at[0, t]) < 0 and int(sim.rvciv_at[0, t]) < 0
...
def first_rival_city(sim):
    idxs = sim.rc_alive[0].nonzero()
    assert len(idxs), "no rival city on this seed/turn"
...
def poke_galley_city(rules, path, GALLEY):
    """1. A GALLEY on a water tile adjacent to a coastal rival city batters it
    (siege → _player_attack_rival_city) and CAPTURES it at 0 HP — naval melee
    ...
    assert wt >= 0, "no free neighbour of the rival city center"
    ...
    d = dir_to(sim, wt, ctr)
    assert d >= 0
    sim.best_melee[0] = 40  # give the empire a real melee CS so the city takes damage
    ...
    # capture: grind to the brink, one more naval melee -> the city joins the empire
    assert bool((~sim.alive[0]).any()), "no free player city slot for the capture"
    sim.rc_hp[0, r, j] = 1
    ...
    assert int(sim.owner[0, ctr]) == c_new, "captured center tile must transfer to the player"
...
def poke_galley_cs(rules, path, GALLEY):
    ...
    d = dir_to(sim, wt, ctr)
    assert d >= 0
    # remove any barb within 2 of the center that could land the kill first
    ...
    assert int(sim.alive[0].sum()) >= ncity0 + 1, "CS capture must found a player city"
    c_new = int(sim.center_at[0, ctr])
...
def poke_quadrireme_unit(rules, path, QUAD, WARRIOR):
    ...
    adjacent rival unit — one roll, NO retaliation, NO advance (r_att path)."""
    sim = build(rules, path)
    for _ in range(25):
        sim.step()
    ...
    assert int(sim.v_hp[0, vslot]) < vhp0, "bombard dealt no damage to the rival unit"
    ...
    print(f"  3 quadrireme bombards a unit OK (rival hp {vhp0}->{int(sim.v_hp[0, vslot])}, ship unharmed, no advance)")
...
def poke_player_naval(rules, path, GALLEY, WARRIOR):
    """5. PLAYER naval end-to-end (forced, since the scripted/controlled player
    builds none): a GALLEY SPAWNS on the nearest free WATER tile, then attacks.
    Plus the documented #50 residual — the RL/controlled MOVE verb reads the
    land `passable` plane (no wpass), so it cannot step a ship onto water."""
    sim = build(rules, path)
    for _ in range(25):
        sim.step()
    r, j, ctr = first_rival_city(sim)
    ...
    force_water(sim, wt)
    n0 = int(sim.p_next[0])
    sim._spawn_player(torch.tensor([True]), torch.tensor([anchor]), torch.tensor([GALLEY]))
    assert int(sim.p_next[0]) == n0 + 1, "player galley failed to spawn"
    gslot = n0
    ...
    # attack: put an at-war rival unit next to the spawned ship, batter it.
    ...
    assert int(sim.v_hp[0, vslot]) < vhp0, "player galley dealt no melee damage"

    # #50 residual: the controlled/RL MOVE verb cannot move a ship onto water.
    ...
    assert int(sim.p_tile[0, gslot]) == before, (
        "RL/controlled move stepped a ship onto water — the #50 residual (player-naval "
        "water-move columns) is unexpectedly LIVE; TS findPath is the naval-aware path"
    )
    print("  5 player naval OK (spawn-on-water + attack; RL water-move is the documented #50 residual)")


def poke_ocean_gate(rules, path, GALLEY):
    """6. OCEAN gate for a naval mover (the _spawn_player naval probe shares the
    ...
    for _ in range(25):
        sim2.step()
    r2, j2, ctr2 = first_rival_city(sim2)
    ...
    sim2._spawn_player(torch.tensor([True]), torch.tensor([anchor2]), torch.tensor([GALLEY]))
...
def poke_walls_pcstk(rules, path, GALLEY, WARRIOR):
    """7a. PLAYER city ANCIENT_WALLS strike (pcstk, in _barbarian_phase): a
    ...
    assert bool(sim.alive[0, 0]), "player capital must be alive"
    ...
    assert tt >= 0
    gslot = place_rmil(sim, r, tt, GALLEY)  # a rival galley in range
    sim.best_melee[0] = 40
    base = sim.snapshot()
    sim._barbarian_phase()
    assert int(sim.v_hp[0, gslot]) < 100, "player city walls did not strike the ship"

    # -- embarked override: a rival WARRIOR at the same tile, embarked vs grounded.
    sim.restore(base)
    clear_all_rival_units(sim)
...
def poke_walls_rcstk(rules, path, GALLEY, WARRIOR):
    """7b. RIVAL city ANCIENT_WALLS strike (rcstk, in _rival_phase): a player
    ship IS struck and an EMBARKED player target takes the flat-CS override.
    Isolation: one at-war rival with a walled city and ZERO units/queue, all
    other rivals at peace, all barbs cleared — only rcstk can touch p_hp."""
    sim = build(rules, path)
    for _ in range(25):
        sim.step()
    r, j, ctr = first_rival_city(sim)
    ...
    base = sim.snapshot()
    sim._rival_phase()
    assert int(sim.p_hp[0, gslot]) < 100, "rival city walls did not strike the player ship"
...
def poke_embarked_capture(rules, path, WARRIOR, BUILDER):
    """8. Capturing an EMBARKED rival civilian: the captured unit appends at the
    player POOL END (p_next) and KEEPS embarked under its new owner (civk path).
    """
    sim = build(rules, path)
    for _ in range(25):
        sim.step()
    ...
    # a TAIL player unit AFTER the builder so pool-end is observable
    ...
    assert bool(sim.p_alive[0, cap]), "captured builder not appended to the player pool"
    ...
    assert not bool(sim.v_alive[0, bslot]), "the rival builder must despawn on capture"
    assert int(sim.pciv_at[0, bt]) == cap, "captured civilian occupies the tile as a player unit"
...
def poke_flank_support(rules, path, GALLEY):
    """9. B-7 flank/support (_flank_support): a NAVAL ally counts for flanking
    (player naval) and support (rival naval); the SAME ally EMBARKED counts 0."""
    sim = build(rules, path)
    for _ in range(25):
        sim.step()
    r = 0
    ...
    for t in range(sim.T):
        ...
        for d in range(6):
            n = int(nb[d])
            if any(int(m[0, n]) >= 0 for m in (sim.pmil_at, sim.pciv_at, sim.rv_at, sim.rvciv_at, sim.barb_at, sim.center_at, sim.cs_at, sim.rvcity_at)):
                ok = False
                break
        if ok:
            dt = t
    ...
    n_sup = int(sim.neigh[dt][0])  # a rival naval ALLY of the rival defender (support)
    n_flk = int(sim.neigh[dt][1])  # a player naval unit HOSTILE to the rival defender (flank)
...
def main() -> None:
    ...
    poke_player_naval(rules, path, GALLEY, WARRIOR)
