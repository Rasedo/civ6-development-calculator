/**
 * AUDIT B-20 (Round B7, re-keyed #73): Great Works. WRITER/MUSICIAN great people no longer
 * apply an instant culture lump — each carries 2 Great Works that fill open
 * AMPHITHEATER (writing, 2) / ART MUSEUM (art, 3) / BROADCAST CENTER (music, 1) slots — the real Civ 6 homes as of #73, lowest city then lowest slot
...
describe('B-20 Great Works', () => {
  ...
  // #73: MUSIC lives in the BROADCAST CENTER, its real Civ 6 home, and that
  ...
  it('#70/S1: a MUSIC work pays double a writing work (4 vs 2) and no gold', () => {
    ...
    expect(queueBuilding(state, city.id, 'BROADCAST_CENTER').ok).toBe(true); // requiresAny MUSEUM
  ...
  // #73: ART is a real kind — the ART MUSEUM's 3 slots, +2 culture apiece, and
  ...
  it('#73: an ARTIST fills the Art Museum (3 slots, +2 culture each)', () => {
    const { state, city } = cityWithAmphitheater();
    expect(queueBuilding(state, city.id, 'MUSEUM').ok).toBe(true);
