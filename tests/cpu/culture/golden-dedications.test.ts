...
import { goldenBoostBonus, goldenCulturePerDistrict, goldenProphetPoints } from '../../../cpu/core/eras';
import { DED_FREE_INQUIRY, DED_PEN_BRUSH_AND_VOICE, DED_EXODUS } from '../../../cpu/data/seats';
import type { GameState } from '../../../cpu/core/types';

// GOLDEN-AGE DEDICATION BONUSES, sourced from the Civ 6 dedication
...
function golden(kind: number): GameState {
  ...
  [2].forEach((v, i) => { const s = seatOf(state, i); if (s) s.age = v; });
  seatOf(state, 0)!.dedicationPicks = [kind];
  return state;
}
...
describe('golden-age dedication bonuses', () => {
  ...
  it('nothing fires outside a GOLDEN age', () => {
    ...
    for (const age of [0, 1]) {
      [age].forEach((v, i) => { const st = seatOf(s, i); if (st) st.age = v; });
  ...
  it('a civ that did not commit the dedication gets nothing', () => {
    ...
    seatOf(s, 0)!.dedicationPicks = [DED_EXODUS];
    expect(goldenBoostBonus(s, 0, false)).toBe(0);
