...
import { tileYields, effectiveAdjacency } from '../src/core/yields';
import { availableTechs, computeUnlocks, makeYieldCtx } from '../src/core/effects';
import { TECHS } from '../src/data/techs';

describe('research progression', () => {
  it('starts with only no-prereq techs available', () => {
    const state = makeState();
    const ids = availableTechs(state).map((t) => t.id).sort();
    expect(ids).toEqual(['ANIMAL_HUSBANDRY', 'ASTROLOGY', 'MINING', 'POTTERY', 'SAILING']);
  });

  it('rejects picking a tech whose prereqs are missing', () => {
    const state = makeState();
    expect(setTechResearch(state, 'WRITING').ok).toBe(false);
    grantTechs(state, 'POTTERY');
    expect(setTechResearch(state, 'WRITING').ok).toBe(true);
  });
...
describe('tech gating and boosts', () => {
  it('gates improvements until researched', () => {
    ...
    expect(validImprovements(state, hills)).toEqual([]); // mine needs Mining
    grantTechs(state, 'MINING');
    expect(validImprovements(state, hills)).toEqual(['MINE']);
  });
  ...
  it('gates feature removal until researched', () => {
    ...
    woods.feature = 'WOODS';
    expect(removeFeature(state, woods.index).ok).toBe(false);
    grantTechs(state, 'MINING');
    expect(removeFeature(state, woods.index).ok).toBe(true);
  });
  ...
  it('hill farms need Civil Engineering', () => {
    ...
    hills.elevation = 'HILLS';
    expect(validImprovements(state, hills)).toEqual([]);
    grantCivics(state, 'CIVIL_ENGINEERING');
    expect(validImprovements(state, hills)).toContain('FARM');
  });
  ...
  it('sandbox ignores research gating', () => {
    ...
    const hills = tileAtCoords(state.map, 9, 8);
    hills.elevation = 'HILLS';
    expect(validImprovements(state, hills)).toContain('MINE');
  });
});
...
describe('governments and policies', () => {
  function withGovernment(govId: string) {
    ...
    expect(setGovernment(state, govId).ok).toBe(true);
    return { state, city };
  }

  it('governments unlock via civics', () => {
    const state = makeState();
    expect(setGovernment(state, 'MONARCHY').ok).toBe(false);
    grantCivics(state, 'DIVINE_RIGHT');
    expect(setGovernment(state, 'MONARCHY').ok).toBe(true);
  });
  ...
  it('policy cards respect slot kinds and uniqueness', () => {
    ...
    expect(setPolicy(state, 0, 'URBAN_PLANNING').ok).toBe(false); // economic into military slot
    expect(setPolicy(state, 1, 'URBAN_PLANNING').ok).toBe(true);
    expect(setPolicy(state, 0, 'VETERANCY').ok).toBe(true);
    // can't slot the same card twice (autocracy has 2 military... use chiefdom's single pair)
    expect(setPolicy(state, 0, 'URBAN_PLANNING').ok).toBe(false);
  });
  ...
  it('Natural Philosophy doubles campus adjacency', () => {
    ...
    expect(setPolicy(state, 0, 'NATURAL_PHILOSOPHY').ok).toBe(true);
  ...
  it('Rationalism boosts campus building yields by 50%', () => {
    ...
    expect(setPolicy(state, 0, 'RATIONALISM').ok).toBe(true);
