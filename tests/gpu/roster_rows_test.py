...
def test_wire(rules, path) -> None:
    ...
    # the district-adjacency family alone: the production rows now carry a
    # continent-keyed member, so a bare count moves with the roster
    assert len(sim._district_adj_rows) == 13
    ...
    _dido = sim._leader_idx("DIDO")
    assert sum(1 for r in sim._route_cap_rows if r[1] == _dido) == 4
