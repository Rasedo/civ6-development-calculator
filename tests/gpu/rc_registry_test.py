...
def radius3_usable(sim, center, exclude=()):
    ...
    for t in range(sim.T):
        ...
        if int(sim.district[0, t]) >= 0 or int(sim.built_wonder[0, t]) >= 0:
            continue
        if int(sim.rc_at[0, t]) >= 0 or int(sim.improvement[0, t]) >= 0:
            continue
        out.append(t)
    return out
...
def poke_capture_luxury_pool(rules, path):
    ...
    t = next(
        ...
        if int(sim.owner[0, tt]) < 0 and int(sim.civ_at[0, tt]) < 0 and int(sim.cs_at[0, tt]) < 0
        and int(sim.lux_id[0, tt]) < 0 and int(sim.district[0, tt]) < 0 and int(sim.improvement[0, tt]) < 0
        and int(sim.center_at[0, tt]) < 0 and int(sim.rc_at[0, tt]) < 0
    )
    sim.lux_id[0, t] = lid
    sim.lux_req[0, t] = req
