"""Gold purchases on seat 0's production head.
...
def scan_to_purchasable(sim) -> list[int]:
    """Scan deterministically for a turn with a purchasable building (idling
    the capital each step keeps it from building the candidates itself). The
    purchase sim and its baseline twin both run THIS exact scan, so they land
    in the identical pre-step state."""
    pb = pbase(sim)
    for _ in range(80):
        sim.step()
        sim.current[:, 0] = -1
...
def test_unit_purchase(rules, path):
    ...
    # Measure the buy against an IDLE step on a twin sim, not against this
    # sim's own pre-step count: the step does more than the purchase (a barb
    # can kill a unit in the same turn), so "total alive went up by one" would
    # conflate the verb under test with the rest of the phase order. The twin
    ...
    assert int(sim.major_unit_alive[0].sum()) == int(twin.major_unit_alive[0].sum()) + 1, "purchased unit did not spawn"
...
def test_settler_sequencing(rules, path):
    ...
    # worked set — a few gold of income side-channel between the two runs.
    # What this poke guards is the SEQUENCING gap (unsequenced reads c1+c1, 72
...
def test_builder_escalation(rules, path):
    """Two same-turn builder purchases price sequentially off the live
    escalator (+4 pre-speed apart) and advance builders_trained."""
    sim = build(rules, path)
    for _ in range(120):
        sim.step()
        if bool(sim.alive[0, 1]):
...
def test_worship_faith_purchase(rules, path):
    ...
    # turn, so a raw before/after delta nets BOTH against the price. The control
