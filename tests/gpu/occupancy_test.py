...
def main() -> None:
    ...
    sim.restore(snap)
    assert int(sim.civ_civilian_at[0, t]) == slot, "civ_civilian_at not in snapshot"
    assert int(sim.civilian_at[0, t]) == slot + sim.POOL_LO["civ"], "civilian_at not in snapshot"
    assert int(sim.civ_unit_charges[0, slot]) == 3, "civ_unit_charges not in snapshot"
    ...
    for _ in range(70):
        sim2.step()
        seen = seen or bool((sim2.civ_civilian_at >= 0).any())
    assert seen, "no civ builder ever existed in 70 turns (B5b broken?)"
    b2 = sim2._builder_idx
    for u in range(int(sim2.civ_unit_next[0])):
        if bool(sim2.civ_unit_alive[0, u]) and int(sim2.civ_unit_type[0, u]) == b2:
            tt2 = int(sim2.civ_unit_tile[0, u])
            assert int(sim2.civ_civilian_at[0, tt2]) == u, "alive builder not indexed by civ_civilian_at"
