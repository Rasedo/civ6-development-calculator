...
def main() -> None:
    ...
    # `relics` and `rc_relics` are the seat-0 and civ VIEWS of one `cty_relics`
    ...
    for p in paths[:4]:
        ...
        if r_pick >= 0:
            break
    assert r_pick >= 0, "no fixture reaches a civ with two cities — cannot exercise compaction"
    if True:
