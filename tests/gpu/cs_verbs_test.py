...
`questSatisfied`):
  ...
  * CIV QUESTS (zero-draw) — one deterministic quest per (civ, CS):
    ...
    NO RNG; completion pays +questEnvoys to that civ's cs_r_envoys.
...
def empty_land_tiles(sim, k: int) -> list[int]:
    ...
    free = (
        ...
        & (sim.pmil_at[0] < 0) & (sim.pciv_at[0] < 0) & (sim.rc_at[0] < 0)
        & (sim.cs_at[0] < 0) & (sim.owner[0] < 0) & (sim.civ_at[0] < 0)
...
def main() -> None:
    ...
    for _ in range(25):
        sim.step()
    assert int(sim.rc_alive[0, R].sum()) >= 1, "civ 0 has no cities after 25 turns"
    ...
    sim._seat_phase()
    assert int(sim.cs_last_levy[0, S0]) == T, "L2: the engine arm refused a stashed levy at peace (at-war is not a rule)"
    ...
    sim.r_treasury[0, R] = cost
    sim._seat_phase()
    assert int(sim.cs_last_levy[0, S0]) != T, "L3: a non-suzerain civ levied"
    ...
    assert int(sim.cs_r_envoys[0, R, S0]) == env0 + q_env, "Q3: questEnvoys not paid to the civ"
    ...
    sim._seat_quest_phase(R, active_mask(sim))
    assert int(sim.cs_r_quest[0, R, s1]) == 0, "Q5: an unmet CS issued a civ quest"
