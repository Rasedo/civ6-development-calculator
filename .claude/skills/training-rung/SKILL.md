...
- O=2 workhorse: `python gpu/train_ppo.py --seats 2 --reward relative
...
- FFA: `--seats 4 --fixtures gpu/fixtures_o4` (+ `--anchor <same-world
  ...
  (generate with `gpu/gen_targets.py`, `--policy <ckpt>` for net-driven
  states).

## Assess (all four, on landing)

1. `python gpu/eval.py --policy <ckpt> --episodes 50` — the standard
   scripted-world number (family context in TRAINING.md).
2. `python gpu/duel_eval.py --a <new> --b <champion>` AND the reverse
   ordering — the seat asymmetry is part of the game; report both.
3. `python gpu/alpharank.py <family checkpoints> --episodes 12` — the
   stationary mass is the ranking; raw win rate misleads under
   intransitivity or seat-trading.
4. `python gpu/behavior_probe.py <prev> <new> --episodes 24 --device cpu`
